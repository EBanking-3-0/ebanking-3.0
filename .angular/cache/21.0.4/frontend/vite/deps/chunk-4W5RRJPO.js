import { withHttpTransferCache } from "./chunk-GHML3VU6.js";
import { CommonModule, PLATFORM_BROWSER_ID } from "./chunk-B6JS5ISQ.js";
import {
  DomAdapter,
  XhrFactory,
  getDOM,
...
import { __spreadValues } from "./chunk-OC4HWNDI.js";
  ...
  if (elements) {
    for (const styleElement of elements) {
      ...
      if (styleElement instanceof HTMLLinkElement) {
        external.set(
          styleElement.href.slice(styleElement.href.lastIndexOf("/") + 1),
          {
            usage: 0,
...
function addBaseHrefToCssSourceMap(baseHref, styles) {
  ...
  return styles.map((cssContent) => {
    ...
    return cssContent.replace(SOURCEMAP_URL_REGEXP, (_, sourceMapUrl) => {
      ...
      const { pathname: resolvedSourceMapUrl } = new URL(
        sourceMapUrl,
        absoluteBaseHrefUrl,
      );
...
var DefaultDomRenderer2 = class {
  ...
  listen(target, event, callback, options) {
    ...
    if (this.tracingService?.wrapEventListener) {
      wrappedCallback = this.tracingService.wrapEventListener(
        target,
        event,
        wrappedCallback,
      );
  ...
  decoratePreventDefault(eventHandler) {
    return (event) => {
      ...
      const allowDefaultBehavior = false
...
var HAMMER_GESTURE_CONFIG = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode ? "HammerGestureConfig" : "",
);
var HAMMER_LOADER = new InjectionToken(
  ...
  return [
    {
      ...
      useValue: () => {
        const isEnabledBlockingInitialNavigation = inject(
          IS_ENABLED_BLOCKING_INITIAL_NAVIGATION,
          {
            optional: true,
          },
        ...
        if (isEnabledBlockingInitialNavigation) {
          ...
          const message = formatRuntimeError(
            5001,
            "Configuration error: found both hydration and enabledBlocking initial navigation in the same application, which is a contradiction.",
          );
...
//# sourceMappingURL=chunk-4W5RRJPO.js.map
