...
function producerAccessed(node) {
  ...
  if (isRecomputing) {
    nextProducerLink =
      prevProducerLink !== void 0
        ? prevProducerLink.nextProducer
        : activeConsumer.producers;
...
function finalizeConsumerAfterComputation(node) {
  ...
  let toRemove =
    producersTail !== void 0 ? producersTail.nextProducer : node.producers;
  if (toRemove !== void 0) {
...
var BASE_EFFECT_NODE = (() =>
  __spreadProps(__spreadValues({}, REACTIVE_NODE), {
...
function throwError(msg, actual, expected, comparison) {
  throw new Error(
    `ASSERTION ERROR: ${msg}` +
      (comparison == null
        ? ""
        : ` [Expected=> ${expected} ${comparison} ${actual} <=Actual]`),
  );
}
...
function assertElement(node) {
  if (!(node instanceof Element)) {
    throwError(
      `The provided value must be an element but got ${stringify(node)}`,
    );
  }
}
...
function getInjectorProfilerContext() {
  !ngDevMode &&
    throwError(
      "getInjectorProfilerContext should never be called in production mode",
    );
...
function setInjectorProfilerContext(context) {
  !ngDevMode &&
    throwError(
      "setInjectorProfilerContext should never be called in production mode",
    );
...
function runInInjectorProfilerContext(injector, token, callback) {
  !ngDevMode &&
    throwError(
      "runInInjectorProfilerContext should never be called in production mode",
    );
...
function isStandalone(type) {
  const def =
    getComponentDef(type) || getDirectiveDef(type) || getPipeDef(type);
...
function cyclicDependencyError(token, path) {
  const message = ngDevMode
    ? `Circular dependency detected for \`${token}\`.`
    : "";
...
function throwInvalidProviderError(ngModuleType, providers, provider) {
  if (ngModuleType && providers) {
    const providerDetail = providers.map((v) =>
      v == provider ? "?" + provider + "?" : "...",
    );
    throw new Error(
      `Invalid provider for the NgModule '${stringify(ngModuleType)}' - only instances of Provider and Type are allowed, got: [${providerDetail.join(", ")}]`,
    );
...
function injectRootLimpMode(token, notFoundValue, flags) {
  ...
  if (injectableDef && injectableDef.providedIn == "root") {
    return injectableDef.value === void 0
...
function assertInjectImplementationNotEqual(fn) {
  ngDevMode &&
    assertNotEqual(
      ...
      fn,
      "Calling ɵɵinject would cause infinite recursion",
    );
}
...
function arrayInsert2(array, index, value1, value2) {
  ngDevMode &&
    assertLessThanOrEqual(index, array.length, "Can't insert past array end.");
...
var R3Injector = class extends EnvironmentInjector {
  ...
  constructor(providers, parent, source, scopes) {
    ...
    this.injectorDefTypes = new Set(
      this.get(INJECTOR_DEF_TYPES, EMPTY_ARRAY, {
        self: true,
      }),
    );
  }
  ...
  get(token, notFoundValue = THROW_IF_NOT_FOUND, options) {
    ...
    try {
      ...
      notFoundValue =
        flags & 8 && notFoundValue === THROW_IF_NOT_FOUND
          ? null
          : notFoundValue;
  ...
  if (isTypeProvider(provider)) {
    ...
    if (isValueProvider(provider)) {
      ...
      if (hasDeps(provider)) {
        ...
        return (
          getFactoryDef(classRef) || injectableDefOrInjectorDefFactory(classRef)
        );
      }
    }
  }
...
function assertTNode(tNode) {
  ...
  if (
    !(
      tNode &&
      typeof tNode === "object" &&
      tNode.hasOwnProperty("directiveStylingLast")
    )
  ) {
...
function assertLViewOrUndefined(value) {
  value &&
    assertEqual(isLView(value), true, "Expecting LView or undefined or null");
}
...
function assertFirstUpdatePass(tView, errMessage) {
  assertEqual(
    ...
    true,
    "Should only be called in first update pass.",
  );
}
...
function assertBetween(lower, upper, index) {
  if (!(lower <= index && index < upper)) {
    throwError(
      `Index out of range (expecting ${lower} <= ${index} < ${upper})`,
    );
  }
}
...
function assertNodeInjector(lView, injectorIndex) {
  ...
  assertNumber(
    lView[injectorIndex + 0],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 1],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 2],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 3],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 4],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 5],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 6],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
    lView[injectorIndex + 7],
    "injectorIndex should point to a bloom filter",
  );
  assertNumber(
...
function getTNode(tView, index) {
  ...
  ngDevMode &&
    assertLessThan(index, tView.data.length, "wrong index for TNode");
...
function requiresRefreshOrTraversal(lView) {
  return !!(
    lView[FLAGS] & (1024 | 8192) || lView[REACTIVE_TEMPLATE_CONSUMER]?.dirty
  );
}
...
(function (CheckNoChangesMode2) {
  ...
  CheckNoChangesMode2[(CheckNoChangesMode2["OnlyDirtyViews"] = 2)] =
    "OnlyDirtyViews";
...
function setCurrentTNode(tNode, isParent) {
  ngDevMode &&
    tNode &&
    assertTNodeForTView(tNode, instructionState.lFrame.tView);
  const lFrame = instructionState.lFrame;
  ...
  if (flags & 4) {
    ...
    while (true) {
      ...
      if (parentTNode === null && !(flags & 1)) {
        ...
        ngDevMode &&
          assertDefined(parentLView, "Parent LView should be defined");
...
function allocLFrame() {
  ...
  const newLFrame =
    childLFrame === null ? createLFrame(currentLFrame) : childLFrame;
  return newLFrame;
}
...
function setSelectedIndex(index) {
  ngDevMode &&
    ...
    assertGreaterThanOrEqual(
      index,
      HEADER_OFFSET,
      "Index must be past HEADER_OFFSET (or -1).",
    );
  ngDevMode &&
    assertLessThan(
      index,
...
var DOCUMENT = new InjectionToken(
...
var NgZone = class _NgZone {
  ...
  static isInAngularZone() {
    return (
      ...
      Zone.current.get(isAngularZoneProperty) === true
    );
  }
...
var globalErrorListeners = new InjectionToken(
...
var SCHEDULE_IN_ROOT_ZONE = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
    ? "run changes outside zone in root"
    : "",
);
...
function effect(effectFn, options) {
  ngDevMode &&
    assertNotInReactiveContext(
      effect,
      "Call `effect` outside of a reactive context. For example, schedule the effect inside the component constructor.",
    );
  ...
  if (viewContext !== null) {
    ...
    if (
      destroyRef instanceof NodeInjectorDestroyRef &&
      destroyRef._lView === viewContext.view
    ) {
      destroyRef = null;
    }
  } else {
...
var EFFECT_NODE = (() =>
  __spreadProps(__spreadValues({}, BASE_EFFECT_NODE), {
    cleanupFns: void 0,
    zone: null,
    onDestroyFns: null,
    run() {
...
var ROOT_EFFECT_NODE = (() =>
  __spreadProps(__spreadValues({}, EFFECT_NODE), {
    consumerMarkedDirty() {
    ...
    function ParamDecoratorFactory(...args) {
      ...
      function ParamDecorator(cls, unusedKey, index) {
        const parameters = cls.hasOwnProperty(PARAMETERS)
          ? cls[PARAMETERS]
          : Object.defineProperty(cls, PARAMETERS, {
              value: [],
            })[PARAMETERS];
...
var ReflectionCapabilities = class {
    ...
    if (tsickleCtorParams && tsickleCtorParams !== parentCtor.ctorParameters) {
      const ctorParameters =
        typeof tsickleCtorParams === "function"
          ? tsickleCtorParams()
          : tsickleCtorParams;
      const paramTypes2 = ctorParameters.map(
        (ctorParam) => ctorParam && ctorParam.type,
      );
      const paramAnnotations2 = ctorParameters.map(
        (ctorParam) =>
          ctorParam &&
          convertTsickleDecoratorIntoMetadata(ctorParam.decorators),
      );
      return this._zipTypesAndAnnotations(paramTypes2, paramAnnotations2);
    }
    const paramAnnotations =
      type.hasOwnProperty(PARAMETERS) && type[PARAMETERS];
    const paramTypes =
      this._reflect &&
  ...
  _ownAnnotations(typeOrFunc, parentCtor) {
    if (
      ...
      typeOrFunc.annotations !== parentCtor.annotations
    ) {
    ...
    if (
      typeOrFunc.decorators &&
      typeOrFunc.decorators !== parentCtor.decorators
    ) {
...
(function (ProfilerEvent2) {
  ProfilerEvent2[(ProfilerEvent2["TemplateCreateStart"] = 0)] =
    "TemplateCreateStart";
  ProfilerEvent2[(ProfilerEvent2["TemplateCreateEnd"] = 1)] =
    "TemplateCreateEnd";
  ProfilerEvent2[(ProfilerEvent2["TemplateUpdateStart"] = 2)] =
    "TemplateUpdateStart";
  ProfilerEvent2[(ProfilerEvent2["TemplateUpdateEnd"] = 3)] =
    "TemplateUpdateEnd";
  ProfilerEvent2[(ProfilerEvent2["LifecycleHookStart"] = 4)] =
    "LifecycleHookStart";
  ...
  ProfilerEvent2[(ProfilerEvent2["BootstrapApplicationStart"] = 8)] =
    "BootstrapApplicationStart";
  ProfilerEvent2[(ProfilerEvent2["BootstrapApplicationEnd"] = 9)] =
    "BootstrapApplicationEnd";
  ProfilerEvent2[(ProfilerEvent2["BootstrapComponentStart"] = 10)] =
    "BootstrapComponentStart";
  ProfilerEvent2[(ProfilerEvent2["BootstrapComponentEnd"] = 11)] =
    "BootstrapComponentEnd";
  ProfilerEvent2[(ProfilerEvent2["ChangeDetectionStart"] = 12)] =
    "ChangeDetectionStart";
  ProfilerEvent2[(ProfilerEvent2["ChangeDetectionEnd"] = 13)] =
    "ChangeDetectionEnd";
  ProfilerEvent2[(ProfilerEvent2["ChangeDetectionSyncStart"] = 14)] =
    "ChangeDetectionSyncStart";
  ProfilerEvent2[(ProfilerEvent2["ChangeDetectionSyncEnd"] = 15)] =
    "ChangeDetectionSyncEnd";
  ProfilerEvent2[(ProfilerEvent2["AfterRenderHooksStart"] = 16)] =
    "AfterRenderHooksStart";
  ProfilerEvent2[(ProfilerEvent2["AfterRenderHooksEnd"] = 17)] =
    "AfterRenderHooksEnd";
  ProfilerEvent2[(ProfilerEvent2["ComponentStart"] = 18)] = "ComponentStart";
  ProfilerEvent2[(ProfilerEvent2["ComponentEnd"] = 19)] = "ComponentEnd";
  ProfilerEvent2[(ProfilerEvent2["DeferBlockStateStart"] = 20)] =
    "DeferBlockStateStart";
  ProfilerEvent2[(ProfilerEvent2["DeferBlockStateEnd"] = 21)] =
    "DeferBlockStateEnd";
  ProfilerEvent2[(ProfilerEvent2["DynamicComponentStart"] = 22)] =
    "DynamicComponentStart";
  ProfilerEvent2[(ProfilerEvent2["DynamicComponentEnd"] = 23)] =
    "DynamicComponentEnd";
  ProfilerEvent2[(ProfilerEvent2["HostBindingsUpdateStart"] = 24)] =
    "HostBindingsUpdateStart";
  ProfilerEvent2[(ProfilerEvent2["HostBindingsUpdateEnd"] = 25)] =
...
function callHooks(currentView, arr, initPhase, currentNodeIndex) {
  ...
  const startIndex =
    currentNodeIndex !== void 0 ? currentView[PREORDER_HOOK_FLAGS] & 65535 : 0;
...
function callHook(currentView, initPhase, arr, i) {
  ...
  if (isInitHook) {
    ...
    if (
      indexWithintInitPhase < currentView[PREORDER_HOOK_FLAGS] >> 16 &&
...
var NodeInjectorFactory = class {
  ...
  constructor(factory, isViewProvider, injectImplementation, name) {
    ...
    ngDevMode &&
      assertEqual(typeof factory, "function", "Expected factory function.");
...
function bloomAdd(injectorIndex, tView, type) {
  ngDevMode &&
    assertEqual(
      tView.firstCreatePass,
      true,
      "expected firstCreatePass to be true",
    );
  let id;
...
function getParentInjectorLocation(tNode, lView) {
  ...
  while (lViewCursor !== null) {
    ...
    ngDevMode &&
      parentTNode &&
      assertTNodeForLView(parentTNode, lViewCursor[DECLARATION_VIEW]);
...
function getOrCreateInjectable(tNode, lView, token, flags = 0, notFoundValue) {
  if (tNode !== null) {
    ...
    const value = lookupTokenUsingNodeInjector(
      tNode,
      lView,
      token,
      flags,
...
function lookupTokenUsingNodeInjector(
  ...
  if (typeof bloomHash === "function") {
    ...
    try {
      ...
      if (ngDevMode) {
        runInInjectorProfilerContext(
          new NodeInjector(getCurrentTNode(), getLView()),
          token,
          () => {
            emitInjectorToCreateInstanceEvent(token);
    ...
    let hostTElementNode =
      flags & 1 ? lView[DECLARATION_COMPONENT_VIEW][T_HOST] : null;
    ...
    while (injectorIndex !== -1) {
      ...
      if (
        ...
        shouldSearchParent(
          flags,
          lView[TVIEW].data[injectorIndex + 8] === hostTElementNode,
        ) &&
...
function locateDirectiveOrProvider(
  ...
  const endIndex = isHostSpecialCase
    ? injectablesStart + cptViewProvidersCount
    : directiveEnd;
...
function bloomHashBitOrFactory(token) {
  ...
  if (typeof tokenId === "number") {
    if (tokenId >= 0) {
      ...
      ngDevMode &&
        assertEqual(tokenId, -1, "Expecting to get Special Injector Id");
  ...
  if (Array.isArray(dep) && dep.length > 0) {
    for (let j = 0; j < dep.length; j++) {
      ...
      if (param instanceof Optional || proto.ngMetadataName === "Optional") {
        ...
        param instanceof SkipSelf ||
        proto.ngMetadataName === "SkipSelf"
      ) {
        meta.skipSelf = true;
...
function getInjectableMetadata(type, srcMeta) {
  ...
  if (
    (isUseClassProvider(meta) || isUseFactoryProvider(meta)) &&
    meta.deps !== void 0
  ) {
...
var QueryList = class {
  ...
  reset(resultsTree, identityAccessor) {
    ...
    if (
      (this._changesDetected = !arrayEquals(
        this._results,
        newResultFlat,
        identityAccessor,
      ))
...
function hasSkipHydrationAttrOnTNode(tNode) {
  ...
  for (let i = 0; i < attrs.length; i += 2) {
    ...
    if (
      typeof value === "string" &&
      value.toLowerCase() === SKIP_HYDRATION_ATTR_NAME_LOWER_CASE
    ) {
      return true;
    }
  }
...
function isI18nInSkipHydrationBlock(parentTNode) {
  return (
    ...
    isInSkipHydrationBlock2(parentTNode)
  );
}
var ChangeDetectionStrategy;
(function (ChangeDetectionStrategy2) {
  ChangeDetectionStrategy2[(ChangeDetectionStrategy2["OnPush"] = 0)] = "OnPush";
  ChangeDetectionStrategy2[(ChangeDetectionStrategy2["Default"] = 1)] =
    "Default";
...
function getLContext(target) {
  ...
  if (mpValue) {
    ...
    while ((parent = parent.parentNode)) {
      ...
      if (parentContext) {
        const lView = Array.isArray(parentContext)
          ? parentContext
          : parentContext.lView;
        if (!lView) {
          return null;
...
function getComponentAtNodeIndex(nodeIndex, lView) {
  const tNode = lView[TVIEW].data[nodeIndex];
  return isComponentHost(tNode)
    ? lView[tNode.directiveStart + tNode.componentOffset]
    : null;
}
...
function getRootView(componentOrLView) {
  ...
  let lView = isLView(componentOrLView)
    ? componentOrLView
...
(function (AcxChangeDetectionStrategy2) {
  AcxChangeDetectionStrategy2[(AcxChangeDetectionStrategy2["Default"] = 0)] =
    "Default";
  AcxChangeDetectionStrategy2[(AcxChangeDetectionStrategy2["OnPush"] = 1)] =
    "OnPush";
  ...
  if (tCleanup && lCleanup) {
    for (let i = 0; i < tCleanup.length; ) {
      ...
      if (typeof firstParam === "string") {
        ...
        const type =
          typeof useCaptureOrIndx === "boolean" || useCaptureOrIndx >= 0
            ? "dom"
            : "output";
        const useCapture =
          typeof useCaptureOrIndx === "boolean" ? useCaptureOrIndx : false;
...
function isDirectiveDefHack(obj) {
  return (
    obj.type !== void 0 &&
    obj.declaredInputs !== void 0 &&
    obj.resolveHostDirectives !== void 0
  );
}
...
var validAppIdInitializer = {
  ...
  useValue: () => {
    ...
    if (!isAlphanumeric) {
      throw new RuntimeError(
        211,
        `APP_ID value "${appId}" is not alphanumeric. The APP_ID must be a string of alphanumeric characters. (a-zA-Z0-9), hyphens (-) and underscores (_) are allowed.`,
      );
    }
  },
};
var PLATFORM_INITIALIZER = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "Platform Initializer" : "",
);
var PLATFORM_ID = new InjectionToken(
...
var ANIMATION_MODULE_TYPE = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "AnimationModuleType" : "",
);
var CSP_NONCE = new InjectionToken(
...
var IMAGE_CONFIG = new InjectionToken(
...
var TransferState = class _TransferState {
  static ɵprov = ɵɵdefineInjectable({
    ...
    factory: () => {
      ...
      if (true) {
        transferState.store = retrieveTransferredState(
          inject2(DOCUMENT),
          inject2(APP_ID),
        );
      }
...
function retrieveTransferredState(doc, appId) {
  ...
  if (script?.textContent) {
    try {
      ...
      console.warn(
        "Exception while restoring TransferState for app " + appId,
        e,
      );
    }
  }
...
var IS_HYDRATION_DOM_REUSE_ENABLED = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
    ? "IS_HYDRATION_DOM_REUSE_ENABLED"
    : "",
);
var PRESERVE_HOST_CONTENT_DEFAULT = false;
var PRESERVE_HOST_CONTENT = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode ? "PRESERVE_HOST_CONTENT" : "",
  {
    factory: () => PRESERVE_HOST_CONTENT_DEFAULT,
  },
);
var IS_I18N_HYDRATION_ENABLED = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
    ? "IS_I18N_HYDRATION_ENABLED"
    : "",
);
var IS_EVENT_REPLAY_ENABLED = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
    ? "IS_EVENT_REPLAY_ENABLED"
    : "",
);
var EVENT_REPLAY_ENABLED_DEFAULT = false;
var IS_INCREMENTAL_HYDRATION_ENABLED = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
    ? "IS_INCREMENTAL_HYDRATION_ENABLED"
    : "",
);
var JSACTION_BLOCK_ELEMENT_MAP = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode
...
var IS_ENABLED_BLOCKING_INITIAL_NAVIGATION = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
...
function setJSActionAttributes(
  nativeElement,
  eventTypes,
  parentDeferBlockId = null,
) {
  ...
  const parts = eventTypes.reduce((prev, curr) => {
    return (existingAttr?.indexOf(curr) ?? -1) === -1
      ? prev + curr + ":;"
      : prev;
  }, "");
...
var JSACTION_EVENT_CONTRACT = new InjectionToken(
...
var DEHYDRATED_BLOCK_REGISTRY = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode
...
var DehydratedBlockRegistry = class _DehydratedBlockRegistry {
  ...
  awaitParentBlock(topmostParentBlock, callback) {
    const parentBlockAwaitCallbacks =
      this.awaitingCallbacks.get(topmostParentBlock) ?? [];
...
function isInternalHydrationTransferStateKey(key) {
  return (
    key === TRANSFER_STATE_TOKEN_ID || key === TRANSFER_STATE_DEFER_BLOCKS_INFO
  );
}
  ...
  if (nghAttrValue !== "") {
    ...
    if (transferState !== null) {
      ...
      ngDevMode &&
        assertDefined(
          data,
          "Unable to retrieve hydration info from the TransferState.",
        );
    }
  }
...
function processTextNodeMarkersBeforeHydration(node) {
  ...
  const commentNodesIterator = doc.createNodeIterator(
    node,
...
function markRNodeAsClaimedByHydration(node, checkIfAlreadyClaimed = true) {
  if (!ngDevMode) {
    throw new Error(
      "Calling `markRNodeAsClaimedByHydration` in prod mode is not supported and likely a mistake.",
    );
  }
...
function markRNodeAsSkippedByHydration(node) {
  if (!ngDevMode) {
    throw new Error(
      "Calling `markRNodeAsSkippedByHydration` in prod mode is not supported and likely a mistake.",
    );
  }
  patchHydrationInfo(node, {
...
function countBlocksSkippedByHydration(injector) {
  ...
  if (ngDevMode) {
    ngDevMode.deferBlocksWithIncrementalHydration =
      Object.keys(nghDeferData).length;
  }
}
function markRNodeAsHavingHydrationMismatch(
  node,
  expectedNodeDetails = null,
  actualNodeDetails = null,
) {
  if (!ngDevMode) {
    throw new Error(
      "Calling `markRNodeAsMismatchedByHydration` in prod mode is not supported and likely a mistake.",
    );
  }
...
function warnIncrementalHydrationNotConfigured() {
  if (!incrementalHydrationEnabledWarned) {
    ...
    console.warn(
      formatRuntimeError(
        508,
        "Angular has detected that some `@defer` blocks use `hydrate` triggers, but incremental hydration was not enabled. Please ensure that the `withIncrementalHydration()` call is added as an argument for the `provideClientHydration()` function call in your application config.",
      ),
    );
  }
}
...
function getParentBlockHydrationQueue(deferBlockId, injector) {
  ...
  while (!isTopMostDeferBlock && currentBlockId) {
    ...
    const hydratingParentBlock =
      dehydratedBlockRegistry.hydrating.get(currentBlockId);
...
function gatherDeferBlocksByJSActionAttribute(doc) {
  ...
  const eventTypes = [
    hoverEventNames.join(":;"),
    interactionEventNames.join(":;"),
  ].join("|");
...
function retrieveDeferBlockDataImpl(injector) {
  ...
  if (transferState !== null) {
    ...
    ngDevMode &&
      assertDefined(
        nghDeferData,
        "Unable to retrieve defer block info from the TransferState.",
      );
    return nghDeferData;
  }
  return {};
...
function getHydrateTimerTrigger(blockData) {
  const trigger = blockData[DEFER_HYDRATE_TRIGGERS]?.find((t) =>
    isTimerTrigger(t),
  );
...
function executeViewQueryFn(flags, viewQueryFn, component) {
  ngDevMode &&
    assertDefined(
      viewQueryFn,
      "View queries function to execute must be defined.",
    );
...
(function (ViewEncapsulation3) {
  ...
  ViewEncapsulation3[
    (ViewEncapsulation3["ExperimentalIsolatedShadowDom"] = 4)
...
function isPropertyValid(element, propName, tagName, schemas) {
  ...
  if (
    matchingSchemas(schemas, tagName) ||
    propName in element ||
  ...
  return (
    typeof Node === "undefined" || Node === null || !(element instanceof Node)
  );
}
...
function getTemplateLocationDetails(lView) {
  ...
  return componentClassName
    ? ` (used in the '${componentClassName}' component template)`
    : "";
}
...
var OPTIONAL_END_TAG_BLOCK_ELEMENTS = tagSet(
  "colgroup,dd,dt,li,p,tbody,td,tfoot,th,thead,tr",
);
var OPTIONAL_END_TAG_INLINE_ELEMENTS = tagSet("rp,rt");
var OPTIONAL_END_TAG_ELEMENTS = merge(
  OPTIONAL_END_TAG_INLINE_ELEMENTS,
  OPTIONAL_END_TAG_BLOCK_ELEMENTS,
);
var BLOCK_ELEMENTS = merge(
  OPTIONAL_END_TAG_BLOCK_ELEMENTS,
  tagSet(
    "address,article,aside,blockquote,caption,center,del,details,dialog,dir,div,dl,figure,figcaption,footer,h1,h2,h3,h4,h5,h6,header,hgroup,hr,ins,main,map,menu,nav,ol,pre,section,summary,table,ul",
  ),
);
var INLINE_ELEMENTS = merge(
  OPTIONAL_END_TAG_INLINE_ELEMENTS,
  tagSet(
    "a,abbr,acronym,audio,b,bdi,bdo,big,br,cite,code,del,dfn,em,font,i,img,ins,kbd,label,map,mark,picture,q,ruby,rp,rt,s,samp,small,source,span,strike,strong,sub,sup,time,track,tt,u,var,video",
  ),
);
var VALID_ELEMENTS = merge(
  VOID_ELEMENTS,
  BLOCK_ELEMENTS,
  INLINE_ELEMENTS,
  OPTIONAL_END_TAG_ELEMENTS,
);
var URI_ATTRS = tagSet(
...
var SKIP_TRAVERSING_CONTENT_IF_INVALID_ELEMENTS = tagSet(
...
function ɵɵvalidateAttribute(value, tagName, attributeName2) {
  ...
  const errorMessage =
    ngDevMode &&
    `Angular has detected that the \`${attributeName2}\` was applied as a binding to the <${tagName}> element${getTemplateLocationDetails(lView)}. For security reasons, the \`${attributeName2}\` can be set on the <${tagName}> element as a static attribute only. 
...
var NG_REFLECT_ATTRS_FLAG = new InjectionToken(
...
function assertStandaloneComponentType(type) {
  ...
  if (!componentDef.standalone) {
    throw new RuntimeError(
      907,
      `The ${stringifyForError(type)} component is not marked as standalone, but Angular expects to have a standalone component here. Please make sure the ${stringifyForError(type)} component does not have the \`standalone: false\` flag in the decorator.`,
    );
  }
}
...
function isNodeMatchingSelector(tNode, selector, isProjectionMode) {
  ...
  const nameOnlyMarkerIdx =
    nodeAttrs !== null ? getNameOnlyMarkerIndex(nodeAttrs) : 0;
  ...
  for (let i = 0; i < selector.length; i++) {
    ...
    if (mode & 4) {
      ...
      if (
        nodeAttrs === null ||
        !isCssClassMatching(tNode, nodeAttrs, current, isProjectionMode)
      ) {
        if (isPositive(mode)) return false;
        skipToNextSelector = true;
      }
...
function createTView(
  ...
  const consts =
    typeof constsOrFactory === "function" ? constsOrFactory() : constsOrFactory;
...
function createLView(
  ...
  if (
    ...
    (parentLView && parentLView[FLAGS] & 2048)
  ) {
    lView[FLAGS] |= 2048;
  }
  ...
  ngDevMode &&
    assertDefined(lView[ENVIRONMENT], "LViewEnvironment is required");
  ...
  ngDevMode &&
    assertEqual(
      tView.type == 2 ? parentLView !== null : true,
      true,
...
(function (InputFlags2) {
  ...
  InputFlags2[(InputFlags2["HasDecoratorInputTransform"] = 2)] =
...
function getLongestComputedTransition(computedStyle) {
  ...
  for (let i = 0; i < transitionedProperties.length; i++) {
    const duration =
      parseCssTimeUnitsToMs(rawDelays[i]) +
      parseCssTimeUnitsToMs(rawDurations[i]);
...
function longestExists(longest) {
  return (
    (longest.animationName != void 0 || longest.propertyName != void 0) &&
    longest.duration > 0
  );
}
function determineLongestAnimationFromComputedStyles(el, animationsMap) {
  ...
  const longest =
    longestAnimation.duration > longestTransition.duration
      ? longestAnimation
      : longestTransition;
...
function determineLongestAnimation(el, animationsMap, areAnimationSupported2) {
  ...
  return animations.length === 0
    ...
    : determineLongestAnimationFromElementAnimations(
        el,
        animationsMap,
        animations,
      );
...
(function (TracingAction2) {
  ...
  TracingAction2[(TracingAction2["AFTER_NEXT_RENDER"] = 1)] =
    "AFTER_NEXT_RENDER";
})(TracingAction || (TracingAction = {}));
var TracingService = new InjectionToken(
...
var AfterRenderSequence = class {
  ...
  destroy() {
    ...
    if (scheduled) {
      this.view[AFTER_RENDER_SEQUENCES_TO_ADD] = scheduled.filter(
        (s) => s !== this,
      );
    }
  }
...
function afterEveryRender(callbackOrSpec, options) {
  ngDevMode &&
    assertNotInReactiveContext(
      afterEveryRender,
      "Call `afterEveryRender` outside of a reactive context. For example, schedule the render callback inside the component constructor`.",
    );
  if (ngDevMode && !options?.injector) {
...
var ANIMATION_QUEUE = new InjectionToken(
...
function runLeaveAnimationsWithCallback(lView, tNode, injector, callback) {
  ...
  if (
    animations == null ||
    animations.leave == void 0 ||
    !animations.leave.has(tNode.index)
  )
    ...
    () => {
      if (animations.leave && animations.leave.has(tNode.index)) {
        ...
        if (leaveAnimations) {
          for (
            let index = 0;
            index < leaveAnimations.animateFns.length;
            index++
          ) {
  ...
  if (destroyHooks !== null) {
    ...
    for (let i = 0; i < destroyHooks.length; i++) {
      ...
      ngDevMode &&
        assertFunction(
          destroyHooksFn,
          "Expecting destroy hook to be a function.",
        );
      destroyHooksFn();
    }
  }
...
function setI18nHandling(
  getInsertInFrontOfRNodeWithI18n2,
  processI18nInsertBefore2,
) {
...
function applyProjectionRecursive(
  ...
  ngDevMode &&
    assertEqual(
      typeof tProjectionNode.projection,
      "number",
      "expecting projection index",
    );
  const nodeToProjectOrRNodes =
...
function applyStyling(renderer, isClassBased, rNode, prop, value) {
  if (isClassBased) {
    ...
    let flags =
      prop.indexOf("-") === -1 ? void 0 : RendererStyleFlags2.DashCase;
    if (value == null) {
      renderer.removeStyle(rNode, prop, flags);
    } else {
      const isImportant =
        typeof value === "string" ? value.endsWith("!important") : false;
      if (isImportant) {
...
function saveResolvedLocalsInData(
  ...
  if (localNames !== null) {
    ...
    for (let i = 0; i < localNames.length; i += 2) {
      ...
      const value =
        index === -1 ? localRefExtractor(tNode, viewData) : viewData[index];
...
function locateHostElement(
  ...
  const preserveHostContent = injector.get(
...
function setPropertyAndInputs(
  ...
  const tView = lView[TVIEW];
  const hasSetInput = setAllInputsForProperty(
    tNode,
    tView,
    lView,
    propName,
...
function setDomProperty(tNode, lView, propName, value, renderer, sanitizer) {
  if (tNode.type & 3) {
    ...
    if (ngDevMode) {
      ...
      if (
        !isPropertyValid(element, propName, tNode.value, lView[TVIEW].schemas)
      ) {
        handleUnknownPropertyError(propName, tNode.value, tNode.type, lView);
      }
    }
    value =
      sanitizer != null ? sanitizer(value, tNode.value || "", propName) : value;
...
function instantiateAllDirectives(tView, lView, tNode) {
  ...
  if (isComponentHost(tNode)) {
    ...
    createComponentLView(
      lView,
      tNode,
      tView.data[start + tNode.componentOffset],
    );
  }
  if (!tView.firstCreatePass) {
...
function storePropertyBindingMetadata(
  ...
  if (tData[bindingIndex] === null) {
    if (
      !tNode.inputs?.[propertyName] &&
      !tNode.hostDirectiveInputs?.[propertyName]
    ) {
      const propBindingIdxs =
        tNode.propertyBindings || (tNode.propertyBindings = []);
...
function setDirectiveInput(tNode, tView, lView, target, publicName, value) {
  ...
  if (ngDevMode && !tNode.directiveToIndex?.has(target.type)) {
    throw new Error(
      `Node does not have a directive with type ${target.type.name}`,
    );
  }
...
function renderComponent(hostLView, componentHostIdx) {
  ngDevMode &&
    assertEqual(
      isCreationMode(hostLView),
      true,
      "Should be run in creation mode",
    );
  ...
  if (hostRNode !== null && componentView[HYDRATION] === null) {
    componentView[HYDRATION] = retrieveHydrationInfo(
      hostRNode,
      componentView[INJECTOR],
    );
  }
...
function renderView(tView, lView, context) {
  ngDevMode &&
    assertEqual(isCreationMode(lView), true, "Should be run in creation mode");
...
function createAndRenderEmbeddedLView(
  declarationLView,
  templateTNode,
  context,
  options,
  ...
  try {
    ...
    ngDevMode &&
      assertDefined(
        embeddedTView,
        "TView must be defined for a template node.",
      );
    ...
    if (declarationViewLQueries !== null) {
      embeddedLView[QUERIES] =
        declarationViewLQueries.createEmbeddedView(embeddedTView);
    }
...
function shouldAddViewToDom(tNode, dehydratedView) {
  return (
    !dehydratedView ||
    dehydratedView.firstChild === null ||
    hasInSkipHydrationBlockFlag(tNode)
  );
}
var USE_EXHAUSTIVE_CHECK_NO_CHANGES_DEFAULT = false;
var UseExhaustiveCheckNoChanges = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode
    ? "exhaustive checkNoChanges"
    : "",
);
...
var REACTIVE_LVIEW_CONSUMER_NODE = __spreadProps(
  __spreadValues({}, REACTIVE_NODE),
  {
    consumerIsAlwaysLive: true,
    kind: "template",
    consumerMarkedDirty: (node) => {
...
function getOrCreateTemporaryConsumer(lView) {
  const consumer =
    lView[REACTIVE_TEMPLATE_CONSUMER] ?? Object.create(TEMPORARY_CONSUMER_NODE);
  consumer.lView = lView;
  return consumer;
}
    ...
    while (requiresRefreshOrTraversal(lView)) {
      if (retries === MAXIMUM_REFRESH_RERUNS$1) {
        throw new RuntimeError(
          103,
          ngDevMode &&
            "Infinite change detection while trying to refresh views. There may be components which each cause the other to require a refresh, causing an infinite loop.",
        );
      }
      retries++;
...
function refreshView(tView, lView, templateFn, context) {
  ngDevMode &&
    assertEqual(isCreationMode(lView), false, "Should be run in update mode");
  if (isDestroyed(lView)) return;
  const flags = lView[FLAGS];
  const isInCheckNoChangesPass = ngDevMode && isInCheckNoChangesMode();
  const isInExhaustiveCheckNoChangesPass =
    ngDevMode && isExhaustiveCheckNoChanges();
  enterView(lView);
...
function markTransplantedViewsForRefresh(lView) {
  for (
    ...
    ngDevMode &&
      assertDefined(
        movedViews,
        "Transplanted View flags set but missing MOVED_VIEWS",
      );
...
function detachView(lContainer, removeIndex) {
  ...
  if (viewToDetach) {
    ...
    const removedLView = removeFromArray(
      lContainer,
      CONTAINER_HEADER_OFFSET + removeIndex,
    );
...
function trackMovedView(declarationContainer, lView) {
  ...
  if (isLView(parent)) {
    ...
    ngDevMode &&
      assertDefined(insertedComponentLView, "Missing insertedComponentLView");
    const declaredComponentLView = lView[DECLARATION_COMPONENT_VIEW];
    ngDevMode &&
      assertDefined(declaredComponentLView, "Missing declaredComponentLView");
...
function validateMatchingNode(
  ...
  if (
    ...
    const hostComponentDef = getDeclarationComponentDef(lView);
    const componentClassName = hostComponentDef?.type?.name;
    const expectedDom = describeExpectedDom(
      lView,
      tNode,
      isViewContainerAnchor,
    );
...
`;
    ...
    const message =
      header + expected + actual + getHydrationAttributeNote() + footer;
...
function invalidSkipHydrationHost(rNode) {
  const header =
    "The `ngSkipHydration` flag is applied on a node that doesn't act as a component host. Hydration can be skipped only on per-component basis.\n\n";
  const actual = `${describeDomFromNode(rNode)}
...
`;
  ...
  content += spacer + "…\n";
  const parentRNode = tNode.type
    ? getParentRElement(lView[TVIEW], tNode, lView)
    : null;
  if (parentRNode) {
  ...
  switch (nodeType) {
    ...
    case Node.TEXT_NODE:
      const content = textContent
        ? ` (with the "${shorten(textContent)}" content)`
        : "";
...
function getHydrationErrorFooter(componentClassName) {
  const componentInfo = componentClassName
...
function shorten(input2, maxLength = 50) {
  ...
  return input2.length > maxLength
    ? `${input2.substring(0, maxLength - 1)}…`
    : input2;
}
...
function processI18nInsertBefore(
  renderer,
  childTNode,
  lView,
  childRNode,
...
function getOrCreateTNode(tView, index, type, name, attrs) {
  ngDevMode &&
    ...
    assertGreaterThanOrEqual(
      index,
      HEADER_OFFSET,
      "TNodes can't be in the LView header.",
    );
...
function createTNode(tView, tParent, type, index, value, attrs) {
  ...
  ngDevMode &&
    assertNotSame(attrs, void 0, "'undefined' is not valid value for 'attrs'");
...
function getTIcu(tView, index) {
  ...
  if (
    ngDevMode &&
    !(
      value.hasOwnProperty("tView") ||
      value.hasOwnProperty("currentCaseLViewIndex")
    )
  ) {
    throwError(
      "We expect to get 'null'|'TIcu'|'TIcuContainer', but got: " + value,
    );
  }
  ...
  if (native === void 0) {
    ...
    if (nodes?.[noOffsetIndex]) {
      ...
      ngDevMode &&
        assertDefined(
          previousTNode,
          "Unexpected state: current TNode does not have a connection to the previous node or a parent node.",
        );
      if (isFirstElementInNgContainer(tNode)) {
        ...
        if (previousTNodeParent) {
          ...
          const segmentHead = getSegmentHead(
            hydrationInfo,
...
function stringifyNavigationInstructions(instructions) {
  ...
  for (let i = 0; i < instructions.length; i += 2) {
    ...
    for (let r = 0; r < repeat; r++) {
      container.push(
        step === NODE_NAVIGATION_STEP_FIRST_CHILD
          ? "firstChild"
          : "nextSibling",
      );
    }
...
function navigateToNode(from, instructions) {
  ...
  for (let i = 0; i < instructions.length; i += 2) {
    ...
    for (let r = 0; r < repeat; r++) {
      if (ngDevMode && !node) {
        throw nodeNotFoundAtPathError(
          from,
          stringifyNavigationInstructions(instructions),
        );
      }
      switch (step) {
...
function locateRNodeByPath(path, lView) {
  const [referenceNode, ...navigationInstructions] =
    decompressNodeLocation(path);
  let ref;
...
function calcPathForNode(tNode, lView, excludedParentNodes) {
  ...
  while (
    parentTNode !== null &&
    (isDisconnectedNode(parentTNode, lView) ||
      excludedParentNodes?.has(parentTNode.index))
  ) {
    parentTNode = parentTNode.parent;
  }
...
function prepareI18nBlockForHydration(
  lView,
  index,
  parentTNode,
  subTemplateIndex,
...
function serializeI18nBlock(lView, serializedI18nBlock, context, nodes) {
  ...
  for (const node of nodes) {
    const nextRNode = serializeI18nNode(
      lView,
      serializedI18nBlock,
      context,
      node,
...
function prepareI18nBlockForHydrationImpl(
  ...
  ngDevMode &&
    assertDefined(
      tI18n,
      "Expected i18n data to be present in a given TView slot during hydration",
    );
  function findHydrationRoot() {
    if (isRootTemplateMessage(subTemplateIndex)) {
      ngDevMode &&
        assertDefined(
          parentTNode,
          "Expected parent TNode while hydrating i18n root",
        );
      const rootNode = locateNextRNode(
        hydrationInfo,
        tView,
        lView,
        parentTNode,
  ...
  ngDevMode &&
    assertDefined(currentNode, "Expected root i18n node during hydration");
  const disconnectedNodes =
    initDisconnectedNodes(hydrationInfo) ?? /* @__PURE__ */ new Set();
  const i18nNodes = (hydrationInfo.i18nNodes ??= /* @__PURE__ */ new Map());
  const caseQueue =
    hydrationInfo.data[I18N_DATA]?.[index - HEADER_OFFSET] ?? [];
  ...
  hydrationInfo.disconnectedNodes =
    disconnectedNodes.size === 0 ? null : disconnectedNodes;
}
function collectI18nNodesFromDom(context, state, nodeOrNodes) {
  if (Array.isArray(nodeOrNodes)) {
    ...
    switch (nodeOrNodes.kind) {
      case 0: {
        const currentNode = appendI18nNodeToCollection(
          context,
          state,
          nodeOrNodes,
        );
          ...
          case 0: {
            ...
            if (isSerializedElementContainer(hydrationInfo, noOffsetIndex)) {
              ...
              collectI18nNodesFromDom(
                ...
                forkHydrationState(
                  state,
                  state.currentNode?.firstChild ?? null,
                ),
                nodeOrNodes.children,
              );
              setCurrentNode(state, currentNode?.nextSibling ?? null);
      ...
      case 3: {
        const selectedCase = state.isConnected
          ? context.caseQueue.shift()
          : null;
        const childState = {
          currentNode: null,
          isConnected: false,
...
function cleanupI18nHydrationData(lView) {
  const hydrationInfo = lView[HYDRATION];
  if (hydrationInfo) {
    const { i18nNodes, dehydratedIcuData: dehydratedIcuDataMap } =
      hydrationInfo;
  ...
  for (const serializedView of serializedViews) {
    for (let i = 0; i < (serializedView[MULTIPLIER] ?? 1); i++) {
      ...
      if (serializedView[NUM_ROOT_NODES] > 0) {
        ...
        currentRNode = siblingAfter(
          serializedView[NUM_ROOT_NODES],
          currentRNode,
        );
      }
...
function findAndReconcileMatchingDehydratedViewsImpl(
  lContainer,
  templateTNode,
  hostLView,
) {
...
function findAndReconcileMatchingDehydratedViews(
  lContainer,
  templateTNode,
  hostLView,
) {
...
function verifyStandaloneImport(depType, importingType) {
  ...
  if (getNgModuleDef(depType) == null) {
    const def =
      getComponentDef(depType) ||
      getDirectiveDef(depType) ||
      getPipeDef(depType);
    if (def != null) {
      if (!def.standalone) {
...
function computeStaticStyling(tNode, attrs, writeToHost) {
  ngDevMode &&
    assertFirstCreatePass(
      getTView(),
      "Expecting to be called in first template pass only",
    );
...
function setupInitialInputs(tNode, directiveIndex, isHostDirective) {
  ...
  while (i < attrs.length) {
    ...
    if (!isHostDirective && inputs.hasOwnProperty(attrName)) {
      ...
      isHostDirective &&
      hostDirectiveInputs.hasOwnProperty(attrName)
    ) {
...
function configureViewWithDirective(tView, tNode, lView, directiveIndex, def) {
  ngDevMode &&
    assertGreaterThanOrEqual(
      directiveIndex,
      HEADER_OFFSET,
      "Must be in Expando section",
    );
  tView.data[directiveIndex] = def;
  const directiveFactory =
    def.factory || (def.factory = getFactoryDef(def.type, true));
...
function assertNoDuplicateDirectives(directives) {
  ...
  for (const current of directives) {
    if (seenDirectives.has(current)) {
      throw new RuntimeError(
        309,
        `Directive ${current.type.name} matches multiple times on the same element. Directives can only match an element once.`,
      );
    }
...
function isListLikeIterable(obj) {
  ...
  return (
    Array.isArray(obj) || (!(obj instanceof Map) && Symbol.iterator in obj)
  );
}
...
function wrapListener(tNode, lView, listenerFn) {
  return function wrapListenerIn_markDirtyAndPreventDefault(event) {
    ...
    let result = executeListenerWithErrorHandling(
      lView,
      context,
      listenerFn,
      event,
...
function listenToDomEvent(
  ...
  if (!eventTargetResolver && isTNodeDirectiveHost) {
    existingListener = findExistingListener(
      tView,
      lView,
      eventName,
      tNode.index,
...
function isAnimationEventType(eventName) {
  return (
    eventName.startsWith("animation") || eventName.startsWith("transition")
  );
}
...
function storeListenerCleanup(
  ...
  const tCleanup = tView.firstCreatePass
    ? getOrCreateTViewCleanup(tView)
    : null;
  const lCleanup = getOrCreateLViewCleanup(lView);
...
function listenToOutput(
  ...
  if (ngDevMode && !isOutputSubscribable(output2)) {
    throw new Error(
      `@Output ${propertyName} not initialized in '${instance.constructor.name}'.`,
    );
  }
...
function inputBindingUpdate(targetDirectiveIdx, publicName, value) {
  ...
  if (bindingUpdated(lView, bindingIndex, value)) {
    ...
    const hasSet = setDirectiveInput(
      tNode,
      tView,
      lView,
      targetDef,
...
function twoWayBinding(publicName, value) {
  ...
  const output2 = outputBinding(publicName + "Change", (eventValue) =>
    value.set(eventValue),
  );
  ngDevMode &&
    assertNotDefined(
      input2.create,
      "Unexpected `create` callback in inputBinding",
    );
  ngDevMode &&
    assertNotDefined(
      output2.update,
      "Unexpected `update` callback in outputBinding",
    );
  const binding = {
    [BINDING]: {
      kind: "twoWay",
      requiredVars:
        input2[BINDING].requiredVars + output2[BINDING].requiredVars,
    },
    set targetIdx(idx) {
...
function createRootViewInjector(componentDef, environmentInjector, injector) {
  ...
  if (realEnvironmentInjector && componentDef.getStandaloneInjector !== null) {
    realEnvironmentInjector =
      componentDef.getStandaloneInjector(realEnvironmentInjector) ||
      realEnvironmentInjector;
  }
...
function createRootLViewEnvironment(rootLViewInjector) {
  ...
  const changeDetectionScheduler = rootLViewInjector.get(
    ChangeDetectionScheduler,
    null,
  );
  let ngReflect = false;
  if (typeof ngDevMode === "undefined" || ngDevMode) {
    ngReflect = rootLViewInjector.get(
      NG_REFLECT_ATTRS_FLAG,
      NG_REFLECT_ATTRS_FLAG_DEFAULT,
    );
  }
  return {
    rendererFactory,
...
var ComponentFactory2 = class extends ComponentFactory$1 {
  ...
  create(
    ...
    try {
      ...
      const hostRenderer = environment.rendererFactory.createRenderer(
        null,
        cmpDef,
      );
      ...
      const hasInputBindings =
        ...
        directives?.some(
          (d) => typeof d !== "function" && d.bindings.some(isInputBinding),
        );
      const rootLView = createLView(
        null,
        rootTView,
        null,
        512 | getInitialLViewFlagsFromDef(cmpDef),
        null,
        null,
        environment,
        hostRenderer,
        rootViewInjector,
        null,
        retrieveHydrationInfo(hostElement, rootViewInjector, true),
      );
...
function createRootTView(
  ...
  if (directives) {
    for (const directive of directives) {
      const directiveType =
        typeof directive === "function" ? directive : directive.type;
      ...
      if (ngDevMode && !directiveDef.standalone) {
        throw new RuntimeError(
          907,
          `The ${stringifyForError(directiveType)} directive must be standalone in order to be applied to a dynamically-created component.`,
        );
      }
...
var ComponentRef2 = class extends ComponentRef$1 {
  ...
  constructor(componentType, _rootLView, _hasInputBindings) {
    ...
    this.instance = getComponentLViewByIndex(this._tNode.index, _rootLView)[
      CONTEXT
    ];
...
function projectNodes(tNode, ngContentSelectors, projectableNodes) {
  ...
  for (let i = 0; i < ngContentSelectors.length; i++) {
    ...
    projection.push(
      nodesforSlot != null && nodesforSlot.length
        ? Array.from(nodesforSlot)
        : null,
    );
  }
}
...
var R3ViewContainerRef = class ViewContainerRef2 extends VE_ViewContainerRef {
  ...
  createComponent(
    ...
    const isComponentFactory =
      componentFactoryOrType && !isType(componentFactoryOrType);
    let index;
    ...
    const componentFactory = isComponentFactory
      ? componentFactoryOrType
      : new ComponentFactory2(getComponentDef(componentFactoryOrType));
    const contextInjector = injector || this.parentInjector;
    if (!environmentInjector && componentFactory.ngModule == null) {
      const _injector = isComponentFactory
        ? contextInjector
        : this.parentInjector;
  ...
  _adjustIndex(index, shift = 0) {
    ...
    if (ngDevMode) {
      assertGreaterThan(
        index,
        -1,
        `ViewRef index must be positive, got ${index}`,
      );
  ...
  embeddedTView(tNode) {
    ...
    for (let i = 0; i < this.length; i++) {
      const childQueryIndex =
        queriesForTemplateRef !== null ? queriesForTemplateRef.length : 0;
      const tqueryClone = this.getByIndex(i).embeddedTView(
        tNode,
        childQueryIndex,
      );
      if (tqueryClone) {
    ...
    return queriesForTemplateRef !== null
      ? new _TQueries_(queriesForTemplateRef)
      : null;
  }
...
function materializeViewResults(tView, lView, tQuery, queryIndex) {
  ...
  if (lQuery.matches === null) {
    ...
    for (
      let i = 0;
      tQueryMatches !== null && i < tQueryMatches.length;
      i += 2
    ) {
  ...
  if (tQueryMatches !== null) {
    ...
    for (let i = 0; i < tQueryMatches.length; i += 2) {
      ...
      if (tNodeIdx > 0) {
        ...
        ngDevMode && assertLContainer(declarationLContainer);
        for (
          let i2 = CONTAINER_HEADER_OFFSET;
          i2 < declarationLContainer.length;
          i2++
        ) {
...
function createContentQuery(directiveIndex, predicate, flags, read) {
  ...
  if (tView.firstCreatePass) {
    ...
    createTQuery(
      tView,
      new TQueryMetadata_(predicate, flags, read),
      tNode.index,
    );
...
function resolveComponentResources(resourceResolver) {
  return __async(this, null, function* () {
    ...
    function cachedResourceResolve(url) {
      ...
      const promise = resourceResolver(url).then((response) =>
        unwrapResponse(url, response),
      );
      urlCache.set(url, promise);
      return promise;
    }
    const resolutionPromises = Array.from(currentQueue).map((_0) =>
      __async(null, [_0], function* ([type, component]) {
        ...
        if (styleUrls?.length) {
          const allFetched = Promise.all(
            styleUrls.map((url) => cachedResourceResolve(url)),
          ).then((fetchedStyles) => {
  ...
  return __async(this, null, function* () {
    ...
    if (response.status !== void 0 && response.status !== 200) {
      throw new RuntimeError(
        918,
        ngDevMode &&
          `Could not load resource: ${url}. Response status: ${response.status}`,
      );
    }
...
function createNgModuleRefWithProviders(
  moduleType,
  parentInjector,
  additionalProviders,
) {
  return new NgModuleRef2(
    moduleType,
    parentInjector,
    additionalProviders,
    false,
...
function ɵɵdefineComponent(componentDefinition) {
  return noSideEffects(() => {
    ...
    const def = __spreadProps(__spreadValues({}, baseDef), {
      ...
      pipeDefs: null,
      dependencies:
        (baseDef.standalone && componentDefinition.dependencies) || null,
      getStandaloneInjector: baseDef.standalone
        ? (parentInjector) => {
            return parentInjector
              .get(StandaloneService)
              .getOrCreateStandaloneInjector(def);
          }
        : null,
...
function getNgDirectiveDef(directiveDefinition) {
  ...
  return {
    ...
    hostDirectives: null,
    inputs: parseAndConvertInputsForDefinition(
      directiveDefinition.inputs,
      declaredInputs,
    ),
...
function getComponentId(componentDef) {
  let hash = 0;
  const componentDefConsts =
    typeof componentDef.consts === "function" ? "" : componentDef.consts;
  const hashSelectors = [
...
function validateHostDirective(hostDirectiveConfig, directiveDef) {
  ...
  if (directiveDef === null) {
    ...
    throw new RuntimeError(
      307,
      `Could not resolve metadata for host directive ${type.name}. Make sure that the ${type.name} class is annotated with an @Directive decorator.`,
    );
  }
  ...
  if (declarationTView.firstCreatePass) {
    ...
    if (getBindingsEnabled()) {
      resolveDirectives(
        ...
        declarationLView,
        tNode,
        getConstant(declarationTView.consts, localRefsIndex),
...
function declareNoDirectiveHostTemplate(
  declarationLView,
  declarationTView,
  index,
  templateFn,
    ...
    if (tNode.tView.ssrId === null) {
      ...
      ngDevMode &&
        assertEqual(
          tNode.tView.ssrId,
          ssrId,
          "Unexpected value of the `ssrId` for this TView",
        );
    }
  }
  if (isNodeCreationMode) {
...
(function (DeferDependenciesLoadingState2) {
  DeferDependenciesLoadingState2[
    (DeferDependenciesLoadingState2["NOT_STARTED"] = 0)
  ] = "NOT_STARTED";
  DeferDependenciesLoadingState2[
    (DeferDependenciesLoadingState2["IN_PROGRESS"] = 1)
  ] = "IN_PROGRESS";
  DeferDependenciesLoadingState2[
    (DeferDependenciesLoadingState2["COMPLETE"] = 2)
  ] = "COMPLETE";
  DeferDependenciesLoadingState2[
    (DeferDependenciesLoadingState2["FAILED"] = 3)
  ] = "FAILED";
...
(function (DeferBlockInternalState2) {
  DeferBlockInternalState2[(DeferBlockInternalState2["Initial"] = -1)] =
    "Initial";
...
function addDepsToRegistry(currentDeps, newDeps) {
  ...
  return currentDeps.length === currentDepSet.size
    ? currentDeps
    : Array.from(currentDepSet);
}
...
function onViewportWrapper(trigger, callback, injector, wrapperOptions) {
  ...
  return onViewport(
    ...
    () => ngZone.run(callback),
    (options) =>
      ngZone.runOutsideAngular(() => createIntersectionObserver(options)),
    wrapperOptions,
  );
}
  ...
  scheduleTimer(ngZone) {
    ...
    if (this.current.length > 0) {
      ...
      if (
        this.timeoutId === null ||
        (this.invokeTimerAt &&
          this.invokeTimerAt - invokeAt > FRAME_DURATION_MS)
      ) {
        this.clearTimeout();
...
var DEFER_BLOCK_DEPENDENCY_INTERCEPTOR = new InjectionToken(
  "DEFER_BLOCK_DEPENDENCY_INTERCEPTOR",
);
var DEFER_BLOCK_CONFIG = new InjectionToken(
...
function createDeferBlockInjector(parentInjector, tDetails, providers) {
  if (parentInjector instanceof ChainedInjector) {
    ...
    const envInjector = getOrCreateEnvironmentInjector(
      parentEnvInjector2,
      tDetails,
      providers,
    );
  ...
  if (
    ...
    const needsScheduling =
      ...
      (getLoadingBlockAfter(tDetails) !== null ||
        getMinimumDurationForState(tDetails, DeferBlockState.Loading) !==
          null ||
...
function findMatchingDehydratedViewForDeferBlock(lContainer, lDetails) {
  const dehydratedViewIx =
    lContainer[DEHYDRATED_VIEWS]?.findIndex(
      (view) => view.data[DEFER_BLOCK_STATE$1] === lDetails[DEFER_BLOCK_STATE],
    ) ?? -1;
  const dehydratedView =
    dehydratedViewIx > -1
      ? lContainer[DEHYDRATED_VIEWS][dehydratedViewIx]
      : null;
  return {
    dehydratedView,
    dehydratedViewIx,
...
function applyDeferBlockState(
  ...
  if (stateTmplIndex !== null) {
    ...
    const { dehydratedView, dehydratedViewIx } =
    ...
    if (
      (newState === DeferBlockState.Complete ||
        newState === DeferBlockState.Error) &&
...
function applyDeferBlockStateWithScheduling(
  ...
  if (
    ...
    lDetails[STATE_IS_FROZEN_UNTIL] <= now
  ) {
    lDetails[STATE_IS_FROZEN_UNTIL] = null;
    const loadingAfter = getLoadingBlockAfter(tDetails);
    const inLoadingAfterPhase = lDetails[LOADING_AFTER_CLEANUP_FN] !== null;
    if (
      newState === DeferBlockState.Loading &&
      loadingAfter !== null &&
      !inLoadingAfterPhase
    ) {
...
function renderDeferStateAfterResourceLoading(tDetails, tNode, lContainer) {
  ngDevMode &&
    assertDefined(
      tDetails.loadingPromise,
      "Expected loading Promise to exist on this defer block",
    );
...
function ɵɵdeferEnableTimerScheduling(
  ...
  if (placeholderConfigIndex != null) {
    tDetails.placeholderBlockConfig = getConstant(
      tViewConsts,
      placeholderConfigIndex,
    );
  }
...
function setClassMetadata(type, decorators, ctorParameters, propDecorators) {
  return noSideEffects(() => {
    ...
    if (propDecorators !== null) {
      if (
        clazz.hasOwnProperty("propDecorators") &&
        clazz.propDecorators !== void 0
      ) {
        clazz.propDecorators = __spreadValues(
          __spreadValues({}, clazz.propDecorators),
          propDecorators,
        );
      } else {
...
function injectorProfilerEventHandler(injectorProfilerEvent) {
  ...
  if (type === 0) {
    ...
    handleInstanceCreatedByInjectorEvent(
      context,
      injectorProfilerEvent.instance,
    );
  } else if (type === 2) {
    handleProviderConfiguredEvent(
      context,
      injectorProfilerEvent.providerRecord,
    );
  } else if (type === 3) {
...
function handleEffectCreatedEvent(context, effect2) {
  ...
  if (diResolver === null) {
    throwError(
      "An EffectCreated event must be run within an injection context.",
    );
  }
  const { resolverToEffects } = frameworkDIDebugData;
  const cleanupContainer =
    effect2 instanceof EffectRefImpl ? effect2[SIGNAL] : effect2.sequence;
...
function handleInjectEvent(context, data) {
  ...
  const diResolverToInstantiatedToken =
    frameworkDIDebugData.resolverToTokenToDependencies;
  if (!diResolverToInstantiatedToken.has(diResolver)) {
    diResolverToInstantiatedToken.set(
      diResolver,
  ...
  const instantiatedTokenToDependencies =
    diResolverToInstantiatedToken.get(diResolver);
...
function handleInstanceCreatedByInjectorEvent(context, data) {
  ...
  if (getDIResolver(context.injector) === null) {
    throwError(
      "An InjectorCreatedInstance event must be run within an injection context.",
    );
  }
  ...
  if (
    standaloneComponent == void 0 ||
    !isStandaloneComponent(standaloneComponent)
  ) {
    return;
  }
...
function handleProviderConfiguredEvent(context, data) {
  ...
  if (diResolver === null) {
    throwError(
      "A ProviderConfigured event must be run within an injection context.",
    );
  }
...
function findDeferBlocks(node, lView, results) {
  ...
  for (const details of blocks) {
    ...
    const data = {
      ...
      triggers: tDetails.debug?.triggers
        ? Array.from(tDetails.debug.triggers).sort()
        : [],
      rootNodes,
    };
    results.push(data);
...
function inferHydrationState(tDetails, lDetails, registry) {
  if (
    ...
    tDetails.hydrateTriggers.has(7)
  ) {
...
function getDependenciesFromInjectable(injector, token) {
  ...
  if (instance === null) {
    throw new Error(
      `Unable to determine instance of ${token} in given injector`,
    );
  }
  const unformattedDependencies = getDependenciesForTokenInInjector(
    token,
    injector,
  );
  ...
  const dependencies = unformattedDependencies.map((dep) => {
    ...
    for (let i = 0; i < resolutionPath.length; i++) {
      ...
      if (
        formattedDependency.flags.host &&
        injectorToCheck instanceof EnvironmentInjector
      ) {
        break;
      }
      ...
      if (instance2 !== null) {
        if (formattedDependency.flags.host) {
          ...
          const lookupFromFirstInjector = firstInjector.get(
            dep.token,
            null,
            __spreadProps(__spreadValues({}, formattedDependency.flags), {
              optional: true,
            }),
          );
...
function getProviderImportPaths(providerImportsContainer) {
  ...
  const visitor = walkProviderTreeToDiscoverImportPaths(
    providerToPath,
    visitedContainers,
  );
  walkProviderTree(
...
function getEnvironmentInjectorProviders(injector) {
  ...
  for (const providerRecord of providerRecordsWithoutImportPaths) {
    ...
    providerRecords.push(
      __spreadProps(__spreadValues({}, providerRecord), {
        importPath,
      }),
    );
  }
...
function getInjectorProviders(injector) {
  ...
  throwError(
    "getInjectorProviders only supports NodeInjector and EnvironmentInjector",
  );
}
    ...
    if (injector instanceof NodeInjector) {
      ...
      if (firstInjector instanceof NodeInjector) {
        ...
        if (moduleInjector === null) {
          throwError(
            "NodeInjector must have some connection to the module injector tree",
          );
        }
...
function getModuleInjectorOfNodeInjector(injector) {
  ...
  if (injector instanceof NodeInjector) {
    ...
    throwError(
      "getModuleInjectorOfNodeInjector must be called with a NodeInjector",
    );
  }
  ...
  for (const [consumer, producers] of signalMap.entries()) {
    ...
    if (isComputedNode(consumer)) {
      ...
      debugSignalGraphNodes.push({
        label:
          consumer.debugName ??
          consumer.lView?.[HOST]?.tagName?.toLowerCase?.(),
        kind: consumer.kind,
        epoch: consumer.version,
...
function extractSignalNodesAndEdgesFromRoots(
  nodes,
  signalDependenciesMap = /* @__PURE__ */ new Map(),
) {
  for (const node of nodes) {
...
function getSignalGraph(injector) {
  ...
  if (
    !(injector instanceof NodeInjector) &&
    !(injector instanceof R3Injector)
  ) {
    return throwError(
      "getSignalGraph must be called with a NodeInjector or R3Injector",
    );
  }
  if (injector instanceof NodeInjector) {
  ...
  const signalDependenciesMap =
    extractSignalNodesAndEdgesFromRoots(signalNodes);
...
function measureEnd(startEvent, entryName, color) {
  ...
  do {
    ...
    assertDefined(
      top,
      "Profiling error: could not find start event entry " + startEvent,
    );
...
function enableProfiling() {
  ...
  if (typeof ngDevMode !== "undefined" && ngDevMode) {
    const removeInjectorProfiler = setInjectorProfiler(
      chromeDevToolsInjectorProfiler,
    );
...
var Testability = class _Testability {
  ...
  static ɵfac = function Testability_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _Testability)(
      ɵɵinject(NgZone),
...
var TestabilityRegistry = class _TestabilityRegistry {
  ...
  findTestabilityInTree(elem, findInAncestors = true) {
    return (
      _testabilityGetter?.findTestabilityInTree(this, elem, findInAncestors) ??
      null
    );
  }
...
var APP_INITIALIZER = new InjectionToken(
...
var APP_BOOTSTRAP_LISTENER = new InjectionToken(
...
function publishSignalConfiguration() {
  setThrowInvalidWriteToSignalError(() => {
    ...
    if (ngDevMode) {
      ...
      errorMessage =
        activeConsumer2 && isReactiveLViewConsumer(activeConsumer2)
...
var ApplicationRef = class _ApplicationRef {
  ...
  bootstrapImpl(
    ...
    return ngZone.run(() => {
      ...
      (typeof ngDevMode === "undefined" || ngDevMode) &&
        warnIfDestroyed(this._destroyed);
      const isComponentFactory =
        componentOrFactory instanceof ComponentFactory$1;
  ...
  synchronize() {
    ...
    if (
      ...
      throw new RuntimeError(
        103,
        ngDevMode &&
          "Infinite change detection while refreshing application views. Ensure views are not calling `markForCheck` on every template execution or that afterRender hooks always mark views for check.",
      );
    }
  }
  synchronizeOnce() {
  ...
  syncDirtyFlagsWithViews() {
    if (
      this.allViews.some(({ _lView }) => requiresRefreshOrTraversal(_lView))
    ) {
      this.dirtyFlags |= 2;
      return;
    } else {
  ...
  _loadComponent(componentRef) {
    ...
    if (ngDevMode && !Array.isArray(listeners)) {
      throw new RuntimeError(
        -209,
        `Unexpected type of the \`APP_BOOTSTRAP_LISTENER\` token value (expected an array, but got ${typeof listeners}). Please check that the \`APP_BOOTSTRAP_LISTENER\` token is configured as a \`multi: true\` provider.`,
      );
    }
...
function triggerResourceLoading(tDetails, lView, tNode) {
  ...
  if (ngDevMode) {
    const deferDependencyInterceptor = injector.get(
      DEFER_BLOCK_DEPENDENCY_INTERCEPTOR,
      null,
      {
        optional: true,
  ...
  tDetails.loadingPromise = Promise.allSettled(dependenciesFn()).then(
    (results) => {
      let failed = false;
      const directiveDefs = [];
      const pipeDefs = [];
      for (const result of results) {
        if (result.status === "fulfilled") {
          ...
          const directiveDef =
            getComponentDef(dependency) || getDirectiveDef(dependency);
          if (directiveDef) {
      ...
      if (failed) {
        ...
        if (tDetails.errorTmplIndex === null) {
          const templateLocation = ngDevMode
            ? getTemplateLocationDetails(lView)
            : "";
          const error = new RuntimeError(
            -750,
            ngDevMode &&
              `Loading dependencies for \`@defer\` block failed, but no \`@error\` block was configured${templateLocation}. Consider using the \`@error\` block to render an error state.`,
          );
          handleUncaughtError(lView, error);
        }
      } else {
...
function triggerHydrationFromBlockName(
  injector,
  blockName,
  replayQueuedEventsFn,
) {
  return __async(this, null, function* () {
    const dehydratedBlockRegistry = injector.get(DEHYDRATED_BLOCK_REGISTRY);
    ...
    const { parentBlockPromise, hydrationQueue } = getParentBlockHydrationQueue(
      blockName,
      injector,
    );
    ...
    if (dehydratedBlockRegistry.has(topmostParentBlock)) {
      yield triggerHydrationForBlockQueue(
        injector,
        hydrationQueue,
        replayQueuedEventsFn,
      );
    } else {
      dehydratedBlockRegistry.awaitParentBlock(topmostParentBlock, () =>
        __async(null, null, function* () {
          return yield triggerHydrationForBlockQueue(
            injector,
            hydrationQueue,
            replayQueuedEventsFn,
          );
...
function triggerHydrationForBlockQueue(
  injector,
  hydrationQueue,
  replayQueuedEventsFn,
) {
  return __async(this, null, function* () {
    ...
    for (
      let blockQueueIdx = 0;
      blockQueueIdx < hydrationQueue.length;
      blockQueueIdx++
    ) {
      const dehydratedBlockId = hydrationQueue[blockQueueIdx];
      const dehydratedDeferBlock =
        dehydratedBlockRegistry.get(dehydratedBlockId);
...
function deferBlockHasErrored(deferBlock) {
  return (
    getLDeferBlockDetails(deferBlock.lView, deferBlock.tNode)[
      DEFER_BLOCK_STATE
...
function logHmrWarning(injector) {
  if (!_hmrWarningProduced) {
    ...
    console2.log(
      formatRuntimeError(
        -751,
        "Angular has detected that this application contains `@defer` blocks and the hot module replacement (HMR) mode is enabled. All `@defer` block dependencies will be loaded eagerly.",
      ),
    );
  }
}
function ɵɵdefer(
  ...
  storeTriggerCleanupFn(0, lDetails, () =>
...
function ɵɵariaProperty(name, value) {
  ...
  if (bindingUpdated(lView, bindingIndex, value)) {
    ...
    ngDevMode &&
      storePropertyBindingMetadata(tView.data, tNode, name, bindingIndex);
  }
...
var ANIMATIONS_DISABLED = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "AnimationsDisabled" : "",
  {
    factory: () => false,
  },
);
var MAX_ANIMATION_TIMEOUT = new InjectionToken(
...
function isLongestAnimation(event, nativeElement) {
  ...
  return (
    ...
    ((longestAnimation.animationName !== void 0 &&
      event.animationName === longestAnimation.animationName) ||
      (longestAnimation.propertyName !== void 0 &&
...
function runEnterAnimation(lView, tNode, value) {
  ...
  const handleEnterAnimationStart = (event) => {
    ...
    const eventName =
      event instanceof AnimationEvent ? "animationend" : "transitionend";
...
function runLeaveAnimations(lView, tNode, value) {
  ...
  (getLViewLeaveAnimations(lView).get(tNode.index).resolvers ??= []).push(
    resolve,
  );
  const activeClasses = getClassListFromValue(value);
  if (activeClasses && activeClasses.length > 0) {
...
function ɵɵcomponentInstance() {
  ...
  ngDevMode &&
    assertDefined(instance, "Expected component instance to be defined");
  return instance;
}
...
function ɵɵcontrol(value, sanitizer) {
  ...
  if (bindingUpdated(lView, bindingIndex, value)) {
    ...
    ngDevMode &&
      storePropertyBindingMetadata(tView.data, tNode, "field", bindingIndex);
  }
...
function initializeControlFirstCreatePass(tView, tNode, lView) {
  ...
  const controlIndex = directiveIndices.find(
    (index) => ɵCONTROL in lView[index],
  );
  ...
  throw new RuntimeError(
    318,
    `${host} is an invalid [field] directive host. The host must be a native form control (such as <input>', '<select>', or '<textarea>') or a custom form control with a 'value' or 'checked' model.`,
  );
}
...
function hasModelInput(directiveDef, name) {
  return (
    hasSignalInput(directiveDef, name) &&
    hasOutput(directiveDef, name + "Change")
  );
}
...
function listenToCustomControl(lView, tNode, control, modelName) {
  ...
  listenToOutput(
    ...
    outputName,
    outputName,
    wrapListener(tNode, lView, (value) =>
      control.state().setControlValue(value),
    ),
  );
...
function ɵɵrepeaterCreate(
  ...
  if (hasEmptyBlock) {
    ngDevMode &&
      assertDefined(
        emptyDecls,
        "Missing number of declarations for the empty repeater block.",
      );
    ngDevMode &&
      assertDefined(
        emptyVars,
        "Missing number of bindings for the empty repeater block.",
      );
    declareNoDirectiveHostTemplate(
      lView,
      tView,
...
var OperationsCounter = class {
  ...
  wasReCreated(collectionLen) {
    return (
      collectionLen > 0 &&
      this.created === this.destroyed &&
      this.created === collectionLen
    );
  }
};
...
function ɵɵrepeater(collection) {
  ...
  try {
    ...
    if (
      ...
      const message = formatRuntimeError(
        -956,
        `The configured tracking expression (track by identity) caused re-creation of the entire collection of size ${liveCollection.length}. This is an expensive operation requiring destruction and subsequent creation of DOM nodes, directives, components etc. Please review the "track expression" and make sure that it uniquely identifies items in a collection.`,
      );
      console.warn(message);
    }
    ...
    if (metadata.hasEmptyBlock) {
      ...
      if (bindingUpdated(hostLView, bindingIndex, isCollectionEmpty)) {
        ...
        if (isCollectionEmpty) {
          const emptyTemplateTNode = getExistingTNode(
            hostTView,
            emptyTemplateIndex,
          );
          const dehydratedView = findAndReconcileMatchingDehydratedViews(
...
function locateOrCreateElementNodeImpl(tView, lView, tNode, name, index) {
  ...
  if (
    hydrationInfo &&
    (hasSkipHydrationAttrOnTNode(tNode) ||
      hasSkipHydrationAttrOnRElement(native))
  ) {
    if (isComponentHost(tNode)) {
...
var _locateOrCreateElementContainerNode = (
  tView,
  lView,
  tNode,
  commentText,
...
(function (LocaleDataIndex2) {
  ...
  LocaleDataIndex2[(LocaleDataIndex2["DayPeriodsFormat"] = 1)] =
    "DayPeriodsFormat";
  LocaleDataIndex2[(LocaleDataIndex2["DayPeriodsStandalone"] = 2)] =
    "DayPeriodsStandalone";
  LocaleDataIndex2[(LocaleDataIndex2["DaysFormat"] = 3)] = "DaysFormat";
  LocaleDataIndex2[(LocaleDataIndex2["DaysStandalone"] = 4)] = "DaysStandalone";
  LocaleDataIndex2[(LocaleDataIndex2["MonthsFormat"] = 5)] = "MonthsFormat";
  LocaleDataIndex2[(LocaleDataIndex2["MonthsStandalone"] = 6)] =
    "MonthsStandalone";
  ...
  LocaleDataIndex2[(LocaleDataIndex2["DateTimeFormat"] = 12)] =
    "DateTimeFormat";
  LocaleDataIndex2[(LocaleDataIndex2["NumberSymbols"] = 13)] = "NumberSymbols";
  LocaleDataIndex2[(LocaleDataIndex2["NumberFormats"] = 14)] = "NumberFormats";
  LocaleDataIndex2[(LocaleDataIndex2["CurrencyCode"] = 15)] = "CurrencyCode";
  LocaleDataIndex2[(LocaleDataIndex2["CurrencySymbol"] = 16)] =
    "CurrencySymbol";
  LocaleDataIndex2[(LocaleDataIndex2["CurrencyName"] = 17)] = "CurrencyName";
  LocaleDataIndex2[(LocaleDataIndex2["Currencies"] = 18)] = "Currencies";
  LocaleDataIndex2[(LocaleDataIndex2["Directionality"] = 19)] =
    "Directionality";
...
(function (I18nCreateOpCode2) {
  ...
  I18nCreateOpCode2[(I18nCreateOpCode2["APPEND_EAGERLY"] = 1)] =
    "APPEND_EAGERLY";
...
function locateOrCreateNodeImpl(lView, index, textOrName, nodeType) {
  ...
  ngDevMode &&
    assertEqual(native.nodeType, nodeType, "expected matching nodeType");
  ngDevMode &&
    nodeType === Node.ELEMENT_NODE &&
    assertEqual(
      native.tagName.toLowerCase(),
...
function applyCreateOpCodes(
  ...
  for (let i = 0; i < createOpCodes.length; i++) {
    ...
    const isComment =
      (opCode & I18nCreateOpCode.COMMENT) === I18nCreateOpCode.COMMENT;
    const appendNow =
  ...
  for (let i = 0; i < mutableOpCodes.length; i++) {
    ...
    if (typeof opCode == "string") {
      ...
      if (lView[textNodeIndex] === null) {
        ...
        lView[textNodeIndex] = _locateOrCreateNode(
          lView,
          textNodeIndex,
          opCode,
          Node.TEXT_NODE,
      ...
      switch (opCode & 1) {
        case 0:
          ...
          if (parentRNode !== null) {
            ...
            ngDevMode &&
              assertGreaterThan(refIdx, HEADER_OFFSET, "Missing ref");
        ...
        case 1:
          ...
          setElementAttribute(
            renderer,
            getNativeByIndex(elementNodeIndex, lView),
            null,
            null,
            attrName,
            attrValue,
          ...
          if (opCode < 0) {
            ...
            switch (opCode & 3) {
              case 1:
                ...
                ngDevMode &&
                  assertDefined(tNodeOrTagName, "Experting TNode or string");
...
function icuCreateOpCodesToString(opcodes) {
  ...
  function consumeOpCode(opCode) {
    ...
    throw new Error(
      "Unexpected OpCode: " + getInstructionFromIcuCreateOpCode(opCode),
    );
  }
  let lastRef = -1;
  while (parser.hasMore()) {
...
function i18nStartFirstCreatePass(
  ...
  for (let i = 0; i < msgParts.length; i++) {
    ...
    if ((i & 1) === 0) {
      ...
      const index2 =
        HEADER_OFFSET + Number.parseInt(value.substring(isClosing ? 2 : 1));
      if (isClosing) {
        existingTNodeStack.shift();
        astStack.shift();
        setCurrentTNode(getCurrentParentTNode(), false);
      } else {
        const tNode = createTNodePlaceholder(
          tView,
          existingTNodeStack[0],
          index2,
        );
...
function parseIcuCase(
  ...
  ngDevMode &&
    assertDefined(inertBodyElement, "Unable to generate inert body element");
  const inertRootNode =
    getTemplateContent(inertBodyElement) || inertBodyElement;
  if (inertRootNode) {
    return walkIcuTree(
          ...
          for (let i = 0; i < elAttrs.length; i++) {
            ...
            if (hasBinding2) {
              if (VALID_ATTRS.hasOwnProperty(lowerAttrName)) {
                ...
                ngDevMode &&
                  console.warn(
                    `WARNING: ignoring unsafe attribute value ${lowerAttrName} on element ${tagName} (see ${XSS_SECURITY_URL})`,
                  );
              }
            } else {
...
function addCreateNodeAndAppend(
  ...
  create.push(
    text,
    createAtIdx,
    icuCreateOpCode(0, appendToParentIdx, createAtIdx),
  );
}
    ...
    result = result.replace(PP_PLACEHOLDERS_REGEXP, (m, phs, tmpl) => {
      ...
      if (!placeholders.length) {
        content.split("|").forEach((placeholder2) => {
          ...
          const isCloseTemplateTag2 =
            PP_CLOSE_TEMPLATE_REGEXP.test(placeholder2);
...
function ɵɵi18nStart(index, messageIndex, subTemplateIndex = -1) {
  ...
  const sameViewParentTNode =
    parentTNode === lView[T_HOST] ? null : parentTNode;
  const parentRNode = getClosestRElement(tView, sameViewParentTNode, lView);
  const insertInFrontOf =
    parentTNode && parentTNode.type & 8 ? lView[parentTNode.index] : null;
  ...
  for (let i = 0; i < projectionSlots.length; i++) {
    ...
    if (
      ngProjectAsAttrVal === null
        ? isNodeMatchingSelectorList(tNode, slotValue, true)
...
function ɵɵprojection(
  ...
  const isEmpty =
    componentHostNode.projection[tProjectionNode.projection] === null;
...
function ɵɵqueryRefresh(queryList) {
  ...
  if (
    queryList.dirty &&
    isCreationMode(lView) === ((tQuery.metadata.flags & 2) === 2)
  ) {
...
function parseClassNameNext(text, index) {
  ...
  index = parserState.keyEnd = consumeClassToken(
    text,
    (parserState.key = index),
    end,
  );
...
function consumeStyleKey(text, startIndex, endIndex) {
  ...
  while (
    startIndex < endIndex &&
    ((ch = text.charCodeAt(startIndex)) === 45 ||
      ch === 95 ||
      ((ch & -33) >= 65 && (ch & -33) <= 90) ||
      (ch >= 48 && ch <= 57))
  ) {
...
function malformedStyleError(text, expecting, index) {
  ngDevMode &&
    assertEqual(typeof text === "string", true, "String expected here");
  throw throwError(
...
function styleStringParser(keyValueArray, text) {
  for (let i = parseStyle(text); i >= 0; i = parseStyleNext(text, i)) {
    styleKeyValueArraySet(
      keyValueArray,
      getLastParsedKey(text),
      getLastParsedValue(text),
    );
  }
}
  ...
  if (value !== NO_CHANGE && bindingUpdated(lView, bindingIndex, value)) {
    ...
    if (
      ...
      updateStylingMap(
        ...
        (lView[bindingIndex + 1] = toStylingKeyValueArray(
          keyValueArraySet2,
          stringParser,
          value,
        )),
...
function stylingFirstUpdatePass(
  ...
  if (tData[bindingIndex + 1] === null) {
    ...
    tStylingKey = wrapInStaticStylingKey(
      tData,
      tNode,
      tStylingKey,
      isClassBased,
...
function wrapInStaticStylingKey(tData, tNode, stylingKey, isClassBased) {
  ...
  if (hostDirectiveDef === null) {
    ...
    if (isFirstStylingInstructionInHostBinding) {
      ...
      if (residual === null) {
        let templateStylingKey = getTemplateHeadTStylingKey(
          tData,
          tNode,
          isClassBased,
        );
        if (
          templateStylingKey !== void 0 &&
...
function updateStylingMap(
  ...
  while (oldKey !== null || newKey !== null) {
    ngDevMode &&
      assertLessThan(oldIndex, 999, "Are we stuck in infinite loop?");
    ngDevMode &&
      assertLessThan(newIndex, 999, "Are we stuck in infinite loop?");
    const oldValue =
    ...
    oldKey =
      oldIndex < oldKeyValueArray.length ? oldKeyValueArray[oldIndex] : null;
    newKey =
      newIndex < newKeyValueArray.length ? newKeyValueArray[newIndex] : null;
  }
}
...
function locateOrCreateTextNodeImpl(tView, lView, tNode, value, index) {
  ...
  ngDevMode &&
    validateMatchingNode(textNative, Node.TEXT_NODE, null, lView, tNode);
...
function interpolationV(lView, values) {
  ...
  for (let i = 1; i < values.length; i += 2) {
    isBindingUpdated =
      bindingUpdated(lView, bindingIndex++, values[i]) || isBindingUpdated;
  }
  ...
  for (let i = 1; i < values.length; i += 2) {
    content +=
      renderStringify(values[i]) +
      (i + 1 !== values.length ? values[i + 1] : "");
  }
  return content;
}
...
function ɵɵtextInterpolate3(prefix, v0, i0, v1, i1, v2, suffix) {
  const lView = getLView();
  const interpolated = interpolation3(
    lView,
    prefix,
    v0,
    i0,
...
function ɵɵtextInterpolate4(prefix, v0, i0, v1, i1, v2, i2, v3, suffix) {
  const lView = getLView();
  const interpolated = interpolation4(
    lView,
    prefix,
    v0,
    i0,
...
function ɵɵtextInterpolate5(
  ...
  const lView = getLView();
  const interpolated = interpolation5(
    lView,
    prefix,
    v0,
    i0,
...
function ɵɵinterpolate(v0) {
  return bindingUpdated(getLView(), nextBindingIndex(), v0)
    ? renderStringify(v0)
    : NO_CHANGE;
}
...
function registerDestroyHooksIfSupported(
  ...
  if (providerIsTypeProvider || providerIsClassProvider) {
    const classToken = providerIsClassProvider
      ? resolveForwardRef(provider.useClass)
      : provider;
    ...
    if (viewProviders) {
      definition.viewProvidersResolver = (def, processProvidersFn) =>
        providersResolver(
          def,
          processProvidersFn
            ? processProvidersFn(viewProviders)
            : viewProviders,
          true,
        );
    }
...
function ɵɵsetNgModuleScope(type, scope) {
  return noSideEffects(() => {
    ...
    ngModuleDef.declarations = convertToTypeArray(
      scope.declarations || EMPTY_ARRAY,
    );
...
function convertToTypeArray(values) {
  ...
  if (flattenValues.some(isForwardRef)) {
    return () =>
      flattenValues.map(resolveForwardRef).map(maybeUnwrapModuleWithProviders);
  } else {
...
function ɵɵpureFunction8(
  ...
  return bindingUpdated4(lView, bindingIndex + 4, exp5, exp6, exp7, exp8) ||
    different
    ? updateBinding(
        lView,
        bindingIndex + 8,
...
function ɵɵpipe(index, pipeName) {
  ...
  const pipeFactory =
    pipeDef.factory || (pipeDef.factory = getFactoryDef(pipeDef.type, true));
  ...
  const previousInjectImplementation =
    setInjectImplementation(ɵɵdirectiveInject);
  try {
...
function getPipeDef2(name, registry) {
  if (registry) {
    if (ngDevMode) {
      ...
      if (pipes.length > 1) {
        console.warn(
          formatRuntimeError(313, getMultipleMatchingPipesMessage(name)),
        );
      }
    }
...
function getMultipleMatchingPipesMessage(name) {
  ...
  const componentInfoMessage = context
    ? ` in the '${context.constructor.name}' component`
    : "";
...
function ɵɵgetComponentDepsFactory(type, rawImports) {
  return () => {
    try {
      return depsTracker.getComponentDependencies(type, rawImports)
        .dependencies;
    } catch (e) {
      console.error(
...
function mergeWithExistingDefinition(currentDef, newDef) {
  ...
  ngDevMode &&
    assertEqual(
      replacement,
      currentDef,
      "Expected definition to be merged in place",
    );
  return {
    newDef: replacement,
    oldDef: clone,
...
function recreateLView(importMeta, id, newDef, oldDef, lView) {
  ...
  ngDevMode && assertTNodeType(tNode, 2);
  ngDevMode &&
    assertNotEqual(newDef, oldDef, "Expected different component definition");
    ...
    try {
      for (let i = moduleQueue.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          ngModule.declarations.every(isResolvedDeclaration)
        ) {
    ...
    get: () => {
      if (ngModuleDef === null) {
        if (
          ...
          throw new Error(
            `'${stringifyForError(moduleType)}' module can't import itself`,
          );
        }
        const compiler = getCompilerFacade({
          usage: 0,
        ...
        ngModuleDef = compiler.compileNgModule(
          angularCoreEnv,
          `ng:///${moduleType.name}/ɵmod.js`,
          {
            type: moduleType,
            bootstrap: flatten(ngModule.bootstrap || EMPTY_ARRAY).map(
              resolveForwardRef,
            ),
...
function verifySemanticsOfNgModuleDef(
  ...
  declarations.forEach((declarationType) =>
    verifyNotStandalone(declarationType, moduleType),
  );
  ...
  declarations.forEach((decl) =>
    verifyDeclarationIsUnique(decl, allowDuplicateDeclarationsInRoot),
  );
  ...
  if (ngModule) {
    ...
    ngModule.bootstrap &&
      deepForEach(ngModule.bootstrap, verifyCorrectBootstrapType);
    ngModule.bootstrap &&
      deepForEach(ngModule.bootstrap, verifyComponentIsPartOfNgModule);
  }
  if (errors.length) {
  ...
  function verifyDirectivesHaveSelector(type) {
    ...
    if (!getComponentDef(type) && def && def.selectors.length == 0) {
      errors.push(
        `Directive ${stringifyForError(type)} has no selector, please add it!`,
      );
    }
  }
  ...
  function verifyDeclarationIsUnique(type, suppressErrors) {
    ...
    if (existingModule && existingModule !== moduleType) {
      if (!suppressErrors) {
        const modules2 = [existingModule, moduleType]
          .map(stringifyForError)
          .sort();
        errors.push(
          `Type ${stringifyForError(type)} is part of the declarations of 2 modules: ${modules2[0]} and ${modules2[1]}! Please consider moving ${stringifyForError(type)} to a higher module that imports ${modules2[0]} and ${modules2[1]}. You can also create a new NgModule that exports and includes ${stringifyForError(type)} then import that NgModule in ${modules2[0]} and ${modules2[1]}.`,
        );
      }
    } else {
  ...
  function verifyCorrectBootstrapType(type) {
    ...
    if (!getComponentDef(type)) {
      errors.push(
        `${stringifyForError(type)} cannot be used as an entry component.`,
      );
    }
    if (isStandalone(type)) {
      errors.push(
        `The \`${stringifyForError(type)}\` class is a standalone component, which can not be used in the \`@NgModule.bootstrap\` array. Use the \`bootstrapApplication\` function for bootstrap instead.`,
      );
    }
  }
  function verifySemanticsOfNgModuleImport(type, importingModule2) {
    ...
    if (directiveDef !== null && !directiveDef.standalone) {
      throw new Error(
        `Unexpected directive '${type.name}' imported by the module '${importingModule2.name}'. Please add an @NgModule annotation.`,
      );
    }
    const pipeDef = getPipeDef(type);
    if (pipeDef !== null && !pipeDef.standalone) {
      throw new Error(
        `Unexpected pipe '${type.name}' imported by the module '${importingModule2.name}'. Please add an @NgModule annotation.`,
      );
    }
  }
}
...
function computeCombinedExports(type) {
  ...
  return flatten(
    maybeUnwrapFn(ngModuleDef.exports).map((type2) => {
...
function patchComponentDefWithScope(componentDef, transitiveScopes) {
  componentDef.directiveDefs = () =>
    Array.from(transitiveScopes.compilation.directives)
      .map((dir) =>
        dir.hasOwnProperty(NG_COMP_DEF)
          ? getComponentDef(dir)
          : getDirectiveDef(dir),
      )
      .filter((def) => !!def);
  componentDef.pipeDefs = () =>
    Array.from(transitiveScopes.compilation.pipes).map((pipe) =>
      getPipeDef(pipe),
    );
      ...
      if (ngComponentDef === null) {
        ...
        const meta = __spreadProps(__spreadValues({}, baseMeta), {
          ...
          hasDirectiveDependencies:
            !baseMeta.isStandalone ||
            (metadata.imports != null && metadata.imports.length > 0),
        });
        compilationDepth++;
        try {
          ...
          if (meta.isStandalone) {
            ...
            const { directiveDefs, pipeDefs } = getStandaloneDefFunctions(
              type,
              imports,
            );
        ...
        if (metadata.schemas) {
          if (meta.isStandalone) {
            ...
            throw new Error(
              `The 'schemas' was specified for the ${stringifyForError(type)} but is only valid on a component that is standalone.`,
            );
          }
...
function getStandaloneDefFunctions(type, imports) {
  ...
  const pipeDefs = () => {
    ...
    const scope = depsTracker.getStandaloneComponentScope(type, imports);
    return [...scope.compilation.pipes]
      .map((p) => getPipeDef(p))
      .filter((d) => d !== null);
  };
  return {
...
function getDirectiveMetadata(type, metadata) {
  ...
  facade.typeSourceSpan = compiler.createParseSourceSpan(
    "Directive",
    name,
    sourceMapUrl,
  );
    ...
    if (propMetadata.hasOwnProperty(field)) {
      ...
      annotations.forEach((ann) => {
        if (isQueryAnn(ann)) {
          if (!ann.selector) {
            throw new Error(
              `Can't construct a query for the property "${field}" of "${stringifyForError(type)}" since the query selector wasn't defined.`,
            );
          }
...
function shouldAddAbstractDirective(type) {
  const reflect = getReflect();
  if (
    LIFECYCLE_HOOKS.some((hookName) => reflect.hasLifecycleHook(type, hookName))
  ) {
    return true;
  }
...
var COMPILER_OPTIONS = new InjectionToken(
...
function trackMicrotaskNotificationForDebugging() {
  ...
  if (
    CONSECUTIVE_MICROTASK_NOTIFICATION_LIMIT -
      consecutiveMicrotaskNotifications <
    5
  ) {
...
var ChangeDetectionSchedulerImpl = class _ChangeDetectionSchedulerImpl {
  ...
  static ɵfac = function ChangeDetectionSchedulerImpl_Factory(
    __ngFactoryType__,
  ) {
...
function provideZonelessChangeDetection() {
  ...
  if (
    ...
    const message = formatRuntimeError(
      914,
      `The application is using zoneless change detection, but is still loading Zone.js. Consider removing Zone.js to get the full benefits of zoneless. In applications using the Angular CLI, Zone.js is typically included in the "polyfills" section of the angular.json file.`,
    );
    console.warn(message);
  }
  return makeEnvironmentProviders([
...
var LOCALE_ID = new InjectionToken(
...
var DEFAULT_CURRENCY_CODE = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "DefaultCurrencyCode" : "",
  {
    factory: () => USD_CURRENCY_CODE,
  },
);
var TRANSLATIONS = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "Translations" : "",
);
var TRANSLATIONS_FORMAT = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode ? "TranslationsFormat" : "",
);
var MissingTranslationStrategy;
(function (MissingTranslationStrategy2) {
  MissingTranslationStrategy2[(MissingTranslationStrategy2["Error"] = 0)] =
    "Error";
  MissingTranslationStrategy2[(MissingTranslationStrategy2["Warning"] = 1)] =
    "Warning";
  MissingTranslationStrategy2[(MissingTranslationStrategy2["Ignore"] = 2)] =
    "Ignore";
...
var DebugNode = class {
  ...
  get listeners() {
    return getListeners(this.nativeNode).filter(
    ...
    for (let i = 0; i < bindingIndexes.length; i++) {
      ...
      if (metadataParts.length > 1) {
        ...
        for (let j = 1; j < metadataParts.length - 1; j++) {
          value +=
            renderStringify(lView[bindingIndex + j - 1]) + metadataParts[j + 1];
        }
  ...
  subscribe(callback) {
    if (this.destroyed) {
      throw new RuntimeError(
        953,
        ngDevMode &&
          "Unexpected subscription to destroyed `OutputRef`. The owning directive/component is destroyed.",
      );
    }
    ...
    if (this.destroyed) {
      console.warn(
        formatRuntimeError(
          953,
          ngDevMode &&
            "Unexpected emit for destroyed `OutputRef`. The owning directive/component is destroyed.",
        ),
      );
      return;
    }
...
var BaseWritableResource = class {
  ...
  constructor(value, debugName) {
    ...
    this.isLoading = computed(
      ...
      ngDevMode ? createDebugNameObject(debugName, "isLoading") : void 0,
    );
  }
...
var ResourceImpl = class extends BaseWritableResource {
  ...
  constructor(request, loaderFn, defaultValue, equal, debugName, injector) {
    ...
    this.unregisterOnDestroy = injector
      .get(DestroyRef)
      .onDestroy(() => this.destroy());
    this.status = computed(
      () => projectStatusOfState(this.state()),
      ngDevMode ? createDebugNameObject(debugName, "status") : void 0,
    );
    this.error = computed(
      () => {
...
var isCaptureEventType = (eventType) =>
...
function createEventInfoFromParameters(
  eventType,
  event,
  targetElement,
  container,
    ...
    if (this.syntheticMouseEventSupport) {
      if (
        ...
        if (
          isMouseSpecialEvent(
            getEvent(eventInfo),
            getEventType(eventInfo),
            getActionElement(action),
          )
        ) {
          const copiedEvent = createMouseSpecialEvent(
            getEvent(eventInfo),
            getActionElement(action),
          );
...
(function (Restriction2) {
  Restriction2[(Restriction2["I_AM_THE_JSACTION_FRAMEWORK"] = 0)] =
...
var Dispatcher = class {
  ...
  dispatch(eventInfo) {
    ...
    if (
      action &&
      shouldPreventDefaultBeforeDispatching(action.element, eventInfoWrapper)
    ) {
...
var PREVENT_DEFAULT_ERROR_MESSAGE_DETAILS =
  " Because event replay occurs after browser dispatch, `preventDefault` would have no effect. You can check whether an event is being replayed by accessing the event phase: `event.eventPhase === EventPhase.REPLAY`.";
var PREVENT_DEFAULT_ERROR_MESSAGE = `\`preventDefault\` called during event replay.`;
var COMPOSED_PATH_ERROR_MESSAGE_DETAILS =
  " Because event replay occurs after browser dispatch, `composedPath()` will be empty. Iterate parent nodes from `event.target` or `event.currentTarget` if you need to check elements in the event path.";
  ...
  dispatchToDelegate(eventInfoWrapper) {
    ...
    while (eventInfoWrapper.getAction()) {
      ...
      if (
        ...
        eventInfoWrapper.getAction().element !==
          eventInfoWrapper.getTargetElement()
      ) {
        return;
      }
      this.dispatchDelegate(
        eventInfoWrapper.getEvent(),
        eventInfoWrapper.getAction().name,
      );
...
function prepareEventForBubbling(eventInfoWrapper) {
  ...
  const originalStopPropagation = eventInfoWrapper
    .getEvent()
    .stopPropagation.bind(event);
  ...
  patchEventInstance(event, "preventDefault", () => {
    ...
    throw new Error(
      PREVENT_DEFAULT_ERROR_MESSAGE +
        (ngDevMode ? PREVENT_DEFAULT_ERROR_MESSAGE_DETAILS : ""),
    );
  });
  patchEventInstance(event, "composedPath", () => {
    throw new Error(
      COMPOSED_PATH_ERROR_MESSAGE +
        (ngDevMode ? COMPOSED_PATH_ERROR_MESSAGE_DETAILS : ""),
    );
  });
}
...
function removeAllEventListeners(earlyJsactionData) {
  ...
  removeEventListeners(
    earlyJsactionData.c,
    earlyJsactionData.et,
    earlyJsactionData.h,
  );
  removeEventListeners(
    earlyJsactionData.c,
    earlyJsactionData.etc,
    earlyJsactionData.h,
    true,
  );
}
...
var EventContract = class _EventContract {
  ...
  addEvent(eventType, prefixedEventType, passive) {
    ...
    if (
      !_EventContract.MOUSE_SPECIAL_SUPPORT &&
      MOUSE_SPECIAL_EVENT_TYPES.indexOf(eventType) >= 0
    ) {
      return;
    }
    ...
    const browserEventType = getBrowserEventType(
      prefixedEventType || eventType,
    );
    if (browserEventType !== eventType) {
      const eventTypes =
        this.browserEventTypeToExtraEventTypes[browserEventType] || [];
  ...
  replayEarlyEventInfos(earlyEventInfos) {
    for (let i = 0; i < earlyEventInfos.length; i++) {
      ...
      const eventTypes = this.getEventTypesForBrowserEventType(
        earlyEventInfo.eventType,
      );
...
(function (R3TemplateDependencyKind2) {
  R3TemplateDependencyKind2[(R3TemplateDependencyKind2["Directive"] = 0)] =
    "Directive";
  R3TemplateDependencyKind2[(R3TemplateDependencyKind2["Pipe"] = 1)] = "Pipe";
  R3TemplateDependencyKind2[(R3TemplateDependencyKind2["NgModule"] = 2)] =
    "NgModule";
  ...
  HOST_TAG_NAME_TOKEN.__NG_ELEMENT_ID__ = (flags) => {
    ...
    if (tNode === null) {
      throw new RuntimeError(
        204,
        ngDevMode &&
          "HOST_TAG_NAME can only be injected in directives and components during construction time (in a class constructor or as a class field initializer)",
      );
    }
    if (tNode.type & 2) {
      return tNode.value;
    ...
    throw new RuntimeError(
      204,
      ngDevMode &&
        `HOST_TAG_NAME was used on ${getDevModeNodeName(tNode)} which doesn't have an underlying element in the DOM. This is invalid, and so the dependency should be marked as optional.`,
    );
  };
...
var Query = class {};
var ContentChildren = makePropDecorator(
  "ContentChildren",
  (selector, opts = {}) =>
    __spreadValues(
      {
...
var ViewChildren = makePropDecorator(
  "ViewChildren",
  (selector, opts = {}) =>
    __spreadValues(
      {
...
var ViewChild = makePropDecorator(
  "ViewChild",
  (selector, opts) =>
    __spreadValues(
      {
...
function compileNgModuleFactory(injector, options, moduleType) {
  ...
  const compilerProviders = compilerOptions.flatMap(
...
var NgZoneChangeDetectionScheduler = class _NgZoneChangeDetectionScheduler {
  ...
  static ɵfac = function NgZoneChangeDetectionScheduler_Factory(
    __ngFactoryType__,
  ) {
...
var PROVIDED_NG_ZONE = new InjectionToken(
  typeof ngDevMode === "undefined" || ngDevMode
...
function internalProvideZoneChangeDetection({
  ...
  ngZoneFactory ??= () =>
    new NgZone(
      __spreadProps(__spreadValues({}, getNgZoneOptions()), {
        scheduleInRootZone,
      }),
    );
  return [
    ...
    },
    {
      ...
      multi: true,
      useFactory: () => {
        const ngZoneChangeDetectionScheduler = inject2(
          NgZoneChangeDetectionScheduler,
          {
            optional: true,
          },
...
function getNgZoneOptions(options) {
  return {
    enableLongStackTrace:
      typeof ngDevMode === "undefined" ? false : !!ngDevMode,
...
var ImagePerformanceWarning = class _ImagePerformanceWarning {
  ...
  start() {
    if (
      ...
      (this.options?.disableImageSizeWarning &&
  ...
  isOversized(image) {
    ...
    if (
      nonOversizedImageExtentions.some((extension) =>
        imageSource.endsWith(extension),
      )
    ) {
      return false;
    }
...
function logOversizedImageWarning(src) {
  console.warn(
    formatRuntimeError(
      -913,
      `An image with src ${src} has intrinsic file dimensions much larger than its rendered size. This can negatively impact application loading performance. For more information about addressing or disabling this warning, see https://angular.dev/errors/NG0913`,
    ),
  );
}
var PLATFORM_DESTROY_LISTENERS = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode
    ? "PlatformDestroyListeners"
    : "",
);
var ENABLE_ROOT_COMPONENT_BOOTSTRAP = new InjectionToken(
  typeof ngDevMode !== "undefined" && ngDevMode
...
function bootstrap(config) {
  ...
  return ngZone.run(() => {
    ...
    const exceptionHandler = envInjector.get(
      INTERNAL_APPLICATION_ERROR_HANDLER,
    );
    if (typeof ngDevMode === "undefined" || ngDevMode) {
      if (
        envInjector.get(PROVIDED_ZONELESS) &&
        envInjector.get(PROVIDED_NG_ZONE)
      ) {
        console.warn(
          formatRuntimeError(
            408,
            "Both provideZoneChangeDetection and provideZonelessChangeDetection are provided. This is likely a mistake. Update the application providers to use only one of the two.",
          ),
        );
      }
    }
    ...
    if (isApplicationBootstrapConfig(config)) {
      ...
      const onPlatformDestroyListeners = config.platformInjector.get(
    ...
    return _callAndReportToErrorHandler(exceptionHandler, ngZone, () => {
      ...
      return initStatus.donePromise
        .then(() => {
          ...
          if (typeof ngDevMode === "undefined" || ngDevMode) {
            const imagePerformanceService = envInjector.get(
              ImagePerformanceWarning,
            );
...
function _moduleDoBootstrap(moduleRef, allPlatformModules) {
  ...
  if (moduleRef._bootstrapComponents.length > 0) {
    ...
    throw new RuntimeError(
      -403,
      ngDevMode &&
        `The module ${stringify(moduleRef.instance.constructor)} was bootstrapped, but it does not declare "@NgModule.bootstrap" components nor a "ngDoBootstrap" method. Please define one of these.`,
    );
  }
...
var PlatformRef = class _PlatformRef {
  ...
  destroy() {
    ...
    this._destroyListeners.forEach((listener) => listener());
    const destroyListeners = this._injector.get(
      PLATFORM_DESTROY_LISTENERS,
      null,
    );
    if (destroyListeners) {
    ...
    typeof ngDevMode === "undefined" || ngDevMode
      ? [
          ...
          options?.interval !== void 0
            ? exhaustiveCheckNoChangesInterval(options.interval)
            : [],
        ]
      : [],
  );
    ...
    while (nextIt || nextRemove) {
      const record =
        ...
        (nextIt &&
          ...
          ? nextIt
          : nextRemove;
      const adjPreviousIndex = getPreviousIndex(
        record,
        addRemoveOffset,
        moveOffsets,
      );
        ...
        if (record.previousIndex == null) {
          ...
          if (localMovePreviousIndex != localCurrentIndex) {
            for (let i = 0; i < localMovePreviousIndex; i++) {
              const offset =
                i < moveOffsets.length ? moveOffsets[i] : (moveOffsets[i] = 0);
  ...
  static extend(factories) {
    return {
      ...
      useFactory: () => {
        ...
        return _IterableDiffers.create(
          factories,
          parent || defaultIterableDiffersFactory(),
        );
      },
    };
  }
  ...
  static extend(factories) {
    return {
      ...
      useFactory: () => {
        ...
        return _KeyValueDiffers.create(
          factories,
          parent || defaultKeyValueDiffersFactory(),
        );
      },
    };
  }
...
function internalCreateApplication(config) {
  const { rootComponent, appProviders, platformProviders, platformRef } =
    config;
  ...
  try {
    const platformInjector =
      platformRef?.injector ?? createOrReusePlatformInjector(platformProviders);
    if (
      (typeof ngDevMode === "undefined" || ngDevMode) &&
    ...
    providers.push(
      ...
      },
      {
        ...
        useFactory: () => {
          ...
          return () => {
            if (
              !shouldEnableEventReplay(injector) ||
              appsWithEventReplay.has(appRef)
            ) {
              return;
            }
            ...
            appRef.whenStable().then(() => {
              ...
              const eventContractDetails = injector.get(
                JSACTION_EVENT_CONTRACT,
              );
              initEventReplay(eventContractDetails, injector);
              const jsActionMap = injector.get(JSACTION_BLOCK_ELEMENT_MAP);
              jsActionMap
                .get(EAGER_CONTENT_LISTENERS_KEY)
...
function invokeRegisteredReplayListeners(injector, event, currentTarget) {
  const blockName =
    (currentTarget &&
      currentTarget.getAttribute(DEFER_BLOCK_SSR_ID_ATTRIBUTE)) ??
    "";
  ...
  for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
    ...
    if (!serializedView) {
      ...
      if (childTView.type === 1) {
        ...
        numRootNodes = calcNumRootNodes(
          childTView,
          childLView,
          childTView.firstChild,
        );
      }
      serializedView = {
      ...
      if (isDeferBlock(lView[TVIEW], tNode)) {
        ...
        const tDetails = getTDeferBlockDetails(lView[TVIEW], tNode);
        if (
          context.isIncrementalHydrationEnabled &&
          tDetails.hydrateTriggers !== null
        ) {
          ...
          const serializedTriggers = serializeHydrateTriggers(
            tDetails.hydrateTriggers,
          );
...
function appendSerializedNodePath(ngh, tNode, lView, excludedParentNodes) {
  const noOffsetIndex = tNode.index - HEADER_OFFSET;
  ngh[NODES] ??= {};
  ngh[NODES][noOffsetIndex] ??= calcPathForNode(
    tNode,
    lView,
    excludedParentNodes,
  );
      ...
      for (const projectionHeadTNode of tNode.projection) {
        ...
        if (!Array.isArray(projectionHeadTNode)) {
          if (
            ...
            !isInSkipHydrationBlock2(projectionHeadTNode)
          ) {
...
function conditionallyAnnotateNodePath(ngh, tNode, lView, excludedParentNodes) {
  ...
  if (
    ...
    !isInSkipHydrationBlock2(tNode.projectionNext)
  ) {
    appendSerializedNodePath(
      ngh,
      tNode.projectionNext,
...
function annotateDeferBlockRootNodesWithJsAction(
  ...
  if (actionList.length > 0) {
    const elementNodes = rootNodes.filter(
...
function printHydrationStats(injector) {
  const console2 = injector.get(Console);
  const message =
    `Angular hydrated ${ngDevMode.hydratedComponents} component(s) and ${ngDevMode.hydratedNodes} node(s), ${ngDevMode.componentsSkippedHydration} component(s) were skipped. ` +
    (isIncrementalHydrationEnabled(injector)
...
function whenStableWithTimeout(appRef) {
  ...
  if (typeof ngDevMode !== "undefined" && ngDevMode) {
    ...
    const timeoutId = ngZone.runOutsideAngular(() => {
      return setTimeout(
        () => logWarningOnStableTimedout(timeoutTime, console2),
        timeoutTime,
      );
    });
    ...
    },
    {
      ...
      useValue: () => {
        ...
        if (inject2(IS_HYDRATION_DOM_REUSE_ENABLED)) {
          ...
          const message = formatRuntimeError(
            -505,
            "Angular hydration was requested on the client, but there was no serialized information present in the server response, thus hydration was not enabled. Make sure the `provideClientHydration()` is included into the list of providers in the server part of the application configuration.",
          );
    ...
    providers.push({
      ...
      useFactory: () => {
        ...
        return () => {
          ...
          const commentsByBlockId = gatherDeferBlocksCommentNodes(
            doc,
            doc.body,
          );
...
function ɵɵngDeclarePipe(decl) {
  ...
  return compiler.compilePipeDeclaration(
    angularCoreEnv,
    `ng:///${decl.type.name}/ɵpipe.js`,
    decl,
  );
}
var NOT_SET = Symbol("NOT_SET");
var EMPTY_CLEANUP_SET = /* @__PURE__ */ new Set();
var AFTER_RENDER_PHASE_EFFECT_NODE = (() =>
  __spreadProps(__spreadValues({}, SIGNAL_NODE), {
  ...
  constructor(impl, effectHooks, view, scheduler, injector, snapshot = null) {
    ...
    for (const phase of AFTER_RENDER_PHASES) {
      ...
      node.registerCleanupFn = (fn) =>
        (node.cleanup ??= /* @__PURE__ */ new Set()).add(fn);
...
function afterRenderEffect(callbackOrSpec, options) {
  ngDevMode &&
    assertNotInReactiveContext(
      afterRenderEffect,
      "Call `afterRenderEffect` outside of a reactive context. For example, create the render effect inside the component constructor`.",
    );
  if (ngDevMode && !options?.injector) {
...
var REQUEST = new InjectionToken(
...
var RESPONSE_INIT = new InjectionToken(
...
var REQUEST_CONTEXT = new InjectionToken(
...
//# sourceMappingURL=chunk-P4KDGOZN.js.map
