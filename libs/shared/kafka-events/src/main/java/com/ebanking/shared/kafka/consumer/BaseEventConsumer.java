...
/**
 * Base class for event consumers. Provides common error handling and acknowledgment patterns.
 *
 * <p>Subclasses should use @KafkaListener annotation and call processEvent() to handle events with
...
public abstract class BaseEventConsumer {
  ...
  /**
   * Processes an event with error handling and acknowledgment. Subclasses should call this method
   * from their @KafkaListener methods.
   *
  ...
  /**
   * Processes an event with error handling and acknowledgment. Includes partition and offset
  ...
  protected void processEvent(
    ...
    try {
      if (partition != null && offset != null) {
        log.debug(
            "Received event: {} from partition {} at offset {}",
            event.getEventType(),
            partition,
            offset);
      } else {
