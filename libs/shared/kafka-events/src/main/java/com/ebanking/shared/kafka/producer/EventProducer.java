...
/**
 * Generic event producer service for publishing Kafka events. Provides methods to publish events to
 * Kafka topics.
 *
...
public class EventProducer {
  ...
  /**
   * Publishes an event synchronously (blocks until completion). Use with caution as it may impact
   * performance.
   *
   * @param topic The Kafka topic name
   * @param event The event to publish
  ...
  public void publishEventSync(String topic, BaseEvent event) {
    ...
    try {
      ...
      log.info(
          "Event published synchronously to topic {}: offset={}",
          topic,
          result.getRecordMetadata().offset());
    } catch (Exception e) {
      log.error("Failed to publish event synchronously to topic {}: {}", topic, e.getMessage(), e);
