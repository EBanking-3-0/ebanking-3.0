...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
     ...
     */
    @Query("SELECT n FROM Notification n WHERE n.status IN ('PENDING', 'RETRYING') AND n.retryCount < :maxRetries")
     ...
     */
    @Query("SELECT n FROM Notification n WHERE n.status IN ('FAILED', 'RETRYING') AND n.retryCount < :maxRetries ORDER BY n.updatedAt ASC")
