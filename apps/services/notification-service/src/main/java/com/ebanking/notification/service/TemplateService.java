...
public class TemplateService {
    ...
    public String renderTemplate(String templateName, Map<String, Object> variables) {
        try {
            ...
            if (templateConfig.getBasePath() != null && !templateName.startsWith(templateConfig.getBasePath())) {
                fullTemplatePath = templateConfig.getBasePath() + templateName;
            }

            String rendered = templateEngine.process(fullTemplatePath, context);
            log.debug("Template {} rendered successfully using base path: {}", templateName, templateConfig.getBasePath());
            return rendered;

        } catch (Exception e) {
            log.error("Failed to render template: {} with base path: {}", templateName, templateConfig.getBasePath(), e);
            throw new RuntimeException("Failed to render template: " + templateName, e);
        }
    }

    /**
     * Render a simple text template by replacing placeholders
     * Format: {{variableName}}
     */
