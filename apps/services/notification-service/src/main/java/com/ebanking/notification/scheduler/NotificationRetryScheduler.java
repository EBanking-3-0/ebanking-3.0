...
public class NotificationRetryScheduler {
  ...
  /**
   * Scheduled task to retry failed notifications. Runs based on the retry delay configuration in
   * milliseconds.
   */
  @Scheduled(
      fixedDelayString = "${notification.template.retry-delay-millis:300000}",
  ...
  public void retryFailedNotifications() {
    ...
    try {
      log.debug(
          "Running notification retry scheduler - Max retries: {}, Delay: {}ms",
          templateConfig.getMaxRetries(),
          templateConfig.getRetryDelayMillis());
