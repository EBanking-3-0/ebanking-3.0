  ...
  public ResponseEntity<ChatResponse> sendMessage(
    ...
    try {
      ...
      if (isNewConversation) {
        eventProducer.publishConversationStarted(
            userId, conversation.getId(), conversation.getSessionId());
      }

      // Save user message
      Message userMessage =
          Message.builder().role(Message.Role.USER).content(request.getMessage()).build();
      conversation = conversationService.addMessage(conversation.getId(), userMessage);

      // Process message with AI
      String memoryId = conversation.getId();
      ChatResponse response = aiService.processMessage(request.getMessage(), userId, memoryId);

      // Publish message received event with response
      eventProducer.publishMessageReceived(
          userId, conversation.getId(), request.getMessage(), response.getResponse());
