...
public class AiService {
  ...
  public ChatResponse processMessage(String userMessage, String userId, String memoryId) {
    try {
      ...
      eventProducer.publishErrorOccurred(
          userId, memoryId, "MESSAGE_PROCESSING_ERROR", e.getMessage());
      return ChatResponse.builder()
          .response(
  ...
  private Map<String, Object> extractParameters(String message, String actionName, String userId) {
    ...
    java.util.regex.Pattern accountIdPattern =
        java.util.regex.Pattern.compile(
            "account[\\s#]*(\\d+)", java.util.regex.Pattern.CASE_INSENSITIVE);
    ...
    java.util.regex.Pattern limitPattern =
        java.util.regex.Pattern.compile(
            "(?:last|recent|limit)[\\s]*(\\d+)", java.util.regex.Pattern.CASE_INSENSITIVE);
