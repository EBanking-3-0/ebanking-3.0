...
/**
 * Component for publishing AI Assistant events using the shared Kafka events library. All events
 * extend BaseEvent and use the standardized EventProducer.
 */
@Slf4j
@Component
...
public class AssistantEventProducer {
  ...
  public void publishMessageReceived(
    ...
    log.info(
        "Published ASSISTANT_MESSAGE_RECEIVED event for user {} in conversation {}",
        userId,
        conversationId);
  }
  ...
  public void publishActionExecuted(
    ...
    log.info(
        "Published ASSISTANT_ACTION_EXECUTED event: {} for user {} (success={})",
        actionName,
        userId,
        success);
  }
  ...
  public void publishConversationStarted(String userId, String conversationId, String sessionId) {
    ...
    log.info(
        "Published ASSISTANT_CONVERSATION_STARTED event for user {}, conversation {}",
        userId,
        conversationId);
  }
  ...
  public void publishErrorOccurred(
      String userId, String conversationId, String errorType, String errorMessage) {
    ...
    log.warn(
        "Published ASSISTANT_ERROR_OCCURRED event: {} for user {} in conversation {}",
        errorType,
        userId,
        conversationId);
  }
