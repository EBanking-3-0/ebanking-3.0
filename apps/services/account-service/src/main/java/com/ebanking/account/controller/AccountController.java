...
public class AccountController {
    ...
    public ResponseEntity<AccountDTO> createAccount(@RequestBody AccountDTO request, Authentication authentication) {
        // In a real app, extract userId from token or look it up. 
        ...
        // For this demo, we'll assume userId is passed in request, but verify it matches token if needed.
    ...
    public ResponseEntity<List<AccountDTO>> getMyAccounts(@RequestParam Long userId) {
        ...
        return ResponseEntity.ok(accountService.getAccountsByUserId(userId).stream()
                .map(this::mapToDTO)
